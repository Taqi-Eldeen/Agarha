...
export function useSearch(params: SearchParams, enabled = true) {
  ...
  return useInfiniteQuery({
    ...
    queryFn: async ({ pageParam, signal }) => {
      const { data } = await api.GET('/v1/search', { params: { query: { ...params, cursor: pageParam } as never }, signal });
...
export function useMapPins(params: SearchParams, enabled = true) {
  ...
  return useQuery({
    ...
    enabled,
    queryFn: async ({ signal }) => (await api.GET('/v1/search/map', { params: { query: params as never }, signal })).data!.items,
    staleTime: 30_000,
  });
}
...
export function useListing(id: string | undefined) {
  ...
  return useQuery({
    ...
    enabled: !!id,
    queryFn: async ({ signal }) => (await api.GET('/v1/listings/{id}', { params: { path: { id: id! } }, signal })).data as ListingDetail,
    staleTime: 60_000,
  });
}
...
export function useDealerProfile(slug: string | undefined) {
  ...
  return useQuery({
    ...
    enabled: !!slug,
    queryFn: async ({ signal }) => (await api.GET('/v1/dealers/{slug}', { params: { path: { slug: slug! } }, signal })).data!,
  });
}

export function useCities() {
  const api = useApi();
  return useQuery({ queryKey: keys.cities, queryFn: async () => (await api.GET('/v1/catalog/cities')).data!.items, staleTime: 3_600_000 });
}
...
export function useContactDealer() {
  const api = useApi();
  return useMutation({
    mutationFn: async (v: { listingId: string; channel: 'whatsapp' | 'call'; locale: 'ar' | 'en' }) => {
      const { data } = await api.POST('/v1/leads', { body: v, headers: { 'idempotency-key': idempotencyKey() } });
...
export function useFavorites(enabled: boolean) {
  const api = useApi();
  return useQuery({ queryKey: keys.favorites, enabled, queryFn: async () => (await api.GET('/v1/me/favorites/cards')).data as unknown as { items: ListingCard[]; unavailableIds: string[] } });
}
...
export function useToggleFavorite() {
  ...
  return useMutation({
    mutationFn: async ({ listingId, on }: { listingId: string; on: boolean; card?: ListingCard }) => {
    ...
    onMutate: async ({ listingId, on, card }) => {
      ...
      const prev = qc.getQueryData<{ items: ListingCard[]; unavailableIds: string[] }>(keys.favorites);
      if (prev) qc.setQueryData(keys.favorites, { ...prev, items: on ? (card ? [card, ...prev.items] : prev.items) : prev.items.filter((c) => c.id !== listingId) });
      return { prev };
    },
