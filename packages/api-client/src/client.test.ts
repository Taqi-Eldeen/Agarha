...
describe('api client', () => {
  it('refreshes once on 401 and retries with the new token (mobile)', async () => {
    ...
    const fetchMock = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (req.url.endsWith('/v1/auth/refresh'))
        return json(200, { tokens: { accessToken: 'new', refreshToken: 'r2' } });
      return req.headers.get('authorization') === 'Bearer new'
        ? json(200, { id: 'u1' })
        : json(401, { code: 'unauthorized', message: 'x', requestId: 'r' });
    });
    const api = createApiClient({
      baseUrl: 'http://api',
      fetch: fetchMock as typeof fetch,
      tokens: { get: async () => tokens, set: async (t) => void (tokens = t) },
    });
  ...
  it('turns API errors into ApiRequestError with code and requestId', async () => {
    ...
    await expect(api.GET('/v1/catalog/cities')).rejects.toMatchObject({
      status: 429,
      code: 'rate_limited',
      requestId: 'req-1',
    });
  ...
  it('sends Accept-Language from the current locale', async () => {
    ...
    const api = createApiClient({
      baseUrl: 'http://api',
      locale: () => 'en',
      fetch: (async (i: RequestInfo | URL) => {
        seen.push((i as Request).headers.get('accept-language') ?? '');
