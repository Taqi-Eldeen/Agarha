...
const withModes: Decorator = (Story, ctx) => {
  ...
  const modes =
    pick && pick !== 'all' ? MODES.filter((m) => `${m.locale}-${m.theme}` === pick) : MODES;
  return (
    <div
      style={{
        display: 'grid',
        gridTemplateColumns: modes.length > 1 ? 'repeat(auto-fit, minmax(360px, 1fr))' : '1fr',
        gap: 16,
      }}
    >
      {modes.map((m) => (
        <div
          key={`${m.locale}-${m.theme}`}
          data-theme={m.theme}
          lang={m.locale}
          dir={m.locale === 'ar' ? 'rtl' : 'ltr'}
          className="bg-page p-4 text-fg"
          style={{
            fontFamily:
              m.locale === 'ar' ? 'IBM Plex Sans Arabic, sans-serif' : 'IBM Plex Sans, sans-serif',
          }}
        >
  ...
  globalTypes: {
    mode: {
      ...
      toolbar: {
        title: 'Mode',
        icon: 'globe',
        items: ['all', 'ar-light', 'ar-dark', 'en-light', 'en-dark'],
        dynamicTitle: true,
      },
    },
  },
