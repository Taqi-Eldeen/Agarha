...
export function FilterChip({ label, selected, onToggle, onRemove, icon }: FilterChipProps) {
    ...
    return (
      <span className="inline-flex h-10 items-center gap-1 rounded-full border border-brand bg-brand-subtle ps-3 text-caption text-fg">
        ...
        <button
          ...
          className="inline-flex size-10 items-center justify-center rounded-full hover:bg-brand/10"
        >
  ...
  return (
    <button
      ...
      onClick={onToggle}
      className={cn(
        'inline-flex h-10 min-h-touch items-center gap-1 rounded-full border px-4 text-caption transition-colors duration-fast',
        selected
          ? 'border-brand bg-brand text-white dark:text-page'
          : 'border-border bg-card text-fg hover:bg-brand-subtle',
      )}
    >
      {icon}
      {label}
