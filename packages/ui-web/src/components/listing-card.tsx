 ...
 */
export function ListingCard({ card, period = 'day', variant = 'list', href, linkAs, onContact, contacting, favorite, priority }: ListingCardProps) {
  const { locale, t, egp, f } = useUi();
  const name = `${card.make[locale]} ${card.model[locale]}`;
  const Link = linkAs ?? (({ href: h, className, children }) => <a href={h} className={className}>{children}</a>);
  const photo = (
    <div className={cn('relative overflow-hidden bg-brand-subtle', variant === 'map-mini' ? 'size-24 shrink-0 rounded-md' : 'aspect-[4/3] w-full')}>
      {card.photo?.url640 ? <BlurImage src={card.photo.url640} srcSet={`${card.photo.url320} 320w, ${card.photo.url640} 640w`} sizes="(min-width: 1024px) 33vw, (min-width: 640px) 50vw, 100vw" blurhash={card.photo.blurhash} alt={`${name} ${card.year}`} priority={priority} /> : null}
      {card.featured && variant !== 'map-mini' ? <Badge kind="featured" className="absolute start-2 top-2" /> : null}
  ...
  return (
    <article className="flex flex-col overflow-hidden rounded-lg border border-border bg-card transition-shadow duration-base hover:shadow-1" aria-label={`${name} ${card.year}`}>
      <Link href={href} className="flex flex-col focus-visible:outline-offset-[-3px]">
        {photo}
        <div className="flex flex-col gap-2 p-4">
          <h3 className="text-body font-semibold">
            {name} <span className="ag-tabular">{card.year}</span> · <span className="font-normal text-fg-secondary">{card.transmission === 'automatic' ? t.automatic : t.manual}</span>
          </h3>
          <PriceTag prices={card.prices} period={period} />
      ...
      {onContact ? (
        <div className="mt-auto grid grid-cols-[3fr_2fr] gap-2 p-4 pt-0 [&>button]:min-w-0 [&>button]:px-2">
          <Button variant="whatsapp" icon={<WhatsAppIcon />} loading={contacting === 'whatsapp'} onClick={() => onContact('whatsapp')}>
            {t.whatsapp}
          </Button>
          <Button variant="secondary" icon={<Phone aria-hidden className="size-5" strokeWidth={1.75} />} loading={contacting === 'call'} onClick={() => onContact('call')}>
            {t.call}
          </Button>
        </div>
      ) : null}
...
export function ListingCardSkeleton() {
  const { t } = useUi();
  return (
    <div role="status" aria-label={t.loading} className="flex animate-pulse flex-col overflow-hidden rounded-lg border border-border bg-card">
