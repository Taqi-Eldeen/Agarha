  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <button
        ...
        className="flex min-h-24 flex-col items-center justify-center gap-1 rounded-lg border-2 border-dashed border-border bg-card p-4 text-brand disabled:opacity-50"
      >
        ...
        {items.map((item, i) => (
          ...
          >
            <div className="relative aspect-[4/3] bg-brand-subtle">
              <img
                src={item.previewUrl}
                alt={f('photoOf', { index: i + 1, total: items.length })}
                className={cn('size-full object-cover', item.status !== 'ready' && 'opacity-70')}
              />
              {i === 0 ? (
              ...
              {item.status === 'uploading' || item.status === 'processing' ? (
                <div
                  ...
                  aria-label={f('uploading', { percent: Math.round((item.progress ?? 0) * 100) })}
                  className="absolute inset-x-0 bottom-0 h-1.5 bg-border"
                >
                  <div
                    className="h-full bg-brand transition-[width] duration-fast"
                    style={{
                      width: `${Math.round((item.progress ?? (item.status === 'processing' ? 1 : 0)) * 100)}%`,
                    }}
                  />
                </div>
              ) : null}
              {item.status === 'failed' ? (
                <p
                  role="alert"
                  className="absolute inset-x-0 bottom-0 bg-danger px-2 py-1 text-caption text-white"
                >
                  {t.uploadFailed}
                </p>
              ) : null}
