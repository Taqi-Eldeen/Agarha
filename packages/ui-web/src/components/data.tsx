      ...
      <table className="w-full border-collapse text-start">
          ...
          <tr className="border-b border-border">
            {columns.map((c) => (
              <th
                ...
                className={cn(
                  'p-3 text-label font-medium uppercase text-fg-secondary',
                ...
                {c.sortValue ? (
                  <button
                    ...
                    onClick={() =>
                      setSort((s) => ({
                        key: c.key,
                        dir: s?.key === c.key && s.dir === 1 ? -1 : 1,
                      }))
                    }
                    aria-label={`${c.header}: ${sort?.key === c.key && sort.dir === 1 ? t.sortDesc : t.sortAsc}`}
                  >
                    {c.header}
                    {sort?.key === c.key ? (
                      sort.dir === 1 ? (
        ...
        <tbody>
          {sorted.map((r) => (
            <tr
              key={rowKey(r)}
              className="flex flex-col border-b border-border p-3 last:border-0 md:table-row md:p-0"
            >
              {columns.map((c) => (
                <td
                  key={c.key}
                  className={cn(
                    'flex justify-between gap-4 py-1 md:table-cell md:p-3',
                    c.numeric && 'ag-tabular md:text-end',
                  )}
                >
    ...
    <div className="flex flex-col gap-1 rounded-lg border border-border bg-card p-4">
      ...
      <p className="ag-tabular font-display text-h1">
        ...
        {trend ? (
          <span
            aria-hidden
            className={cn('ms-2 text-caption', trend === 'up' ? 'text-available' : 'text-danger')}
          >
            {trend === 'up' ? '▲' : '▼'}
          </span>
        ) : null}
