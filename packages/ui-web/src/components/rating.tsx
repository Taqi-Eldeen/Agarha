      ...
      <div role="radiogroup" aria-label={f('rating', { rating: value })} className="flex gap-1">
        {[1, 2, 3, 4, 5].map((n) => (
          <button
            ...
            aria-label={f('rating', { rating: n })}
            onClick={() => onChange(n)}
            className="inline-flex size-12 items-center justify-center"
          >
            <Star
              aria-hidden
              className={cn(cls, n <= value ? 'fill-featured text-featured' : 'text-border')}
              strokeWidth={1.75}
            />
          </button>
        ))}
    ...
    <span role="img" aria-label={f('rating', { rating: value })} className="inline-flex gap-0.5">
      {[1, 2, 3, 4, 5].map((n) => (
        <Star
          ...
          className={cn(
            cls,
            n <= Math.round(value) ? 'fill-featured text-featured' : 'text-border',
          )}
          strokeWidth={1.75}
        />
      ))}
