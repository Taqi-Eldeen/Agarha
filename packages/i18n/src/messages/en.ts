// English catalog. Must have exactly the same keys as ar.ts (enforced by the Messages type).
import type { Messages } from './ar.js';

export const en: Messages = {
  common: {
    appName: 'Agarha',
    tagline: 'Rent a car with the facts upfront',
    languageSwitch: 'العربية',
    forDealers: 'For dealers',
    signIn: 'Sign in',
    signOut: 'Sign out',
    retry: 'Try again',
    loading: 'Loading…',
    comingSoon: 'We are getting the first verified dealers ready. Check back soon.',
  },
  price: {
    perDay: '{price} / day',
    perWeek: '{price} / week',
    perMonth: '{price} / month',
    deposit: 'Deposit {amount}',
  },
  freshness: {
    fresh: 'Confirmed {ago}',
    aging: 'Confirmed {ago}',
    stale: 'Last confirmed {ago}',
  },
  safety: {
    neverPayDeposit: 'Never pay a deposit before seeing the car.',
    notAParty: 'Agarha is not a party to any rental agreement.',
  },
  auth: {
    phoneLabel: 'Mobile number',
    phoneHint: 'Egyptian mobile, for example 010 1234 5678',
    sendCode: 'Send code',
    codeLabel: 'Verification code',
    codeSentSms: 'We sent a 6-digit code by SMS to {phone}.',
    codeSentWhatsapp: 'We sent a 6-digit code on WhatsApp to {phone}.',
    verify: 'Verify',
    resendIn: 'Resend in {seconds}s',
    resend: 'Resend code',
    useWhatsapp: 'Send on WhatsApp instead',
  },
  otpMessage: {
    body: 'Your Agarha code is {code}. It expires in {minutes} minutes. Never share it with anyone.',
  },
  notifications: {
    leadAlertTitle: 'New enquiry from Agarha',
    leadAlertBody: 'A customer contacted you on {channel} about {car}. Ref {ref}.',
    availabilityNudgeTitle: 'Are your cars still available?',
    availabilityNudgeBody: '{count} cars need confirming. Confirm them in one tap to stay visible to customers.',
    reviewPromptTitle: 'How was {dealer}?',
    reviewPromptBody: 'You contacted {dealer} about {car}. Rate your experience to help others.',
    savedSearchTitle: 'New cars in your search',
    savedSearchBody: '{count} new cars match "{name}".',
    dealerVerifiedTitle: 'Your business is verified',
    dealerVerifiedBody: 'Congratulations! Your business is verified and you can publish cars now.',
    dealerRejectedTitle: 'We need different documents',
    dealerRejectedBody: 'We could not verify your business: {reason}. Upload the documents again from onboarding.',
    availabilityRequestTitle: 'Availability request',
    availabilityRequestBody: 'A customer asks if {car} is free from {from} to {to}. Ref {ref}.',
    availabilityAnswerYesTitle: 'The car is available',
    availabilityAnswerNoTitle: 'The car is not available',
    availabilityAnswerBody: '{dealer} answered your request about {car}. Message them on WhatsApp to arrange it.',
    listingHiddenTitle: 'Listing hidden',
    listingHiddenBody: '{car} was hidden because it was not confirmed for 14 days. Confirm it to show it again.',
    invoicePaidTitle: 'Payment received',
    invoicePaidBody: 'We received {amount} for invoice {number}. Thank you.',
  },
  ui: {
    close: 'Close',
    loading: 'Loading…',
    required: 'Required',
    optional: 'Optional',
    remove: 'Remove',
    undo: 'Undo',
    retry: 'Try again',
    next: 'Next',
    back: 'Back',
    save: 'Save',
    cancel: 'Cancel',
    confirm: 'Confirm',
    whatsapp: 'WhatsApp',
    call: 'Call',
    verified: 'Verified',
    featured: 'Featured',
    fresh: 'Confirmed available',
    stale: 'Needs confirming',
    withDriver: 'With driver',
    selfDrive: 'Self-drive',
    selfOrDriver: 'Self-drive or with driver',
    available: 'Available',
    unavailable: 'Not available right now',
    availabilityUpdated: 'Availability updated',
    confirmedAgo: 'Confirmed {ago}',
    deposit: 'Deposit',
    noDeposit: 'No deposit',
    minAge: 'Age {age}+',
    kmPerDay: '{km} km per day',
    unlimitedKm: 'Unlimited km',
    airportPickup: 'Airport pickup',
    seats: '{count} seats',
    automatic: 'Automatic',
    manual: 'Manual',
    photoOf: 'Photo {index} of {total}',
    previousPhoto: 'Previous photo',
    nextPhoto: 'Next photo',
    noPhotos: 'No photos yet',
    rating: 'Rated {rating} out of 5',
    reviewsCount: '{count} reviews',
    dealerReply: 'Reply from the dealer',
    uploadPhotos: 'Add photos',
    uploadHint: 'Up to 12 photos, each under 10 MB',
    uploading: 'Uploading {percent}%',
    uploadFailed: 'Upload failed',
    makeCover: 'Make cover photo',
    moveEarlier: 'Move earlier',
    moveLater: 'Move later',
    step: 'Step {current} of {total}',
    otpDigit: 'Digit {index} of the code',
    phonePrefix: 'Egypt +20',
    searchPlaceholder: 'Search a model or area',
    noResults: 'No results',
    selectPlaceholder: 'Choose',
    emptyTitle: 'Nothing here yet',
    errorTitle: 'Something went wrong',
    map: 'Map',
    zoomIn: 'Zoom in',
    zoomOut: 'Zoom out',
    clusterCount: '{count} cars',
    perDay: '/ day',
    perWeek: '/ week',
    perMonth: '/ month',
    views: 'Views',
    contacts: 'Contacts',
    sortAsc: 'Sort ascending',
    sortDesc: 'Sort descending',
    docs: {
      national_id: 'National ID',
      passport: 'Passport',
      egyptian_driving_licence: 'Egyptian driving licence',
      international_driving_permit: 'International driving permit',
      foreign_driving_licence: 'Foreign driving licence',
      utility_bill: 'Utility bill',
      employment_letter: 'Employment letter',
    },
    bodyTypes: {
      sedan: 'Sedan',
      hatchback: 'Hatchback',
      suv: 'SUV',
      crossover: 'Crossover',
      minivan: 'Minivan',
      van: 'Van',
      pickup: 'Pickup',
      coupe: 'Coupe',
      convertible: 'Convertible',
      luxury: 'Luxury',
    },
  },
  errors: {
    validation_failed: 'Please check the highlighted fields.',
    required: 'This field is required.',
    too_short: 'Too short. Please add a little more.',
    too_long: 'Too long. Please shorten it.',
    invalid_format: 'That format is not right. Please check it.',
    invalid_number: 'Enter a whole number.',
    invalid_cr: 'The commercial registration number must be 3 to 30 letters or digits.',
    invalid_tax_card: 'The tax card number must be 9 to 15 digits.',
    week_price_exceeds_seven_days: 'The weekly price must be at most 7 × the daily price.',
    month_price_exceeds_thirty_one_days: 'The monthly price must be at most 31 × the daily price.',
    end_before_start: 'The end date must be on or after the start date.',
    lat_and_lng_together: 'Set the full branch location on the map.',
    invalid_egypt_mobile: 'Enter an Egyptian mobile number, for example 010 1234 5678.',
    rate_limited: 'Too many attempts. Please wait a few minutes and try again.',
    captcha_failed: 'We could not confirm you are human. Please try again.',
    otp_invalid: 'That code is not right. Check the message and try again.',
    otp_expired: 'That code has expired. Request a new one.',
    otp_too_many_attempts: 'Too many wrong codes. Request a new one.',
    otp_delivery_failed: 'We could not send the code. Try WhatsApp or try again shortly.',
    unauthorized: 'Please sign in to continue.',
    forbidden: 'You do not have access to this.',
    not_found: 'We could not find that.',
    internal_error: 'Something went wrong on our side. Please try again.',
  },
};
