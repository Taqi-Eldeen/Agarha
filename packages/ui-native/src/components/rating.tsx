...
export function RatingStars({
  ...
  const star = (n: number, filled: boolean) => (
    <Star
      size={px}
      color={filled ? colors.accentFeatured : colors.borderDefault}
      fill={filled ? colors.accentFeatured : 'transparent'}
      strokeWidth={1.75}
    />
  );
  if (onChange)
    return (
      <View
        ...
        accessibilityLabel={f('rating', { rating: value })}
      ...
      >
        {[1, 2, 3, 4, 5].map((n) => (
          <Pressable
            ...
            accessibilityLabel={f('rating', { rating: n })}
            onPress={() => onChange(n)}
