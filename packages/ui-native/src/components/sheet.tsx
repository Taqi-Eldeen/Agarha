  ...
  return (
    ...
    >
      ...
      <KeyboardAvoidingView
        ...
        className="max-h-[90%] rounded-t-lg bg-card"
        style={{ direction: dir }}
      >
        <View
          accessibilityRole="header"
          className="flex-row items-center justify-between border-b border-border px-4 py-2"
        >
          <Text variant="h2">{title}</Text>
          <IconButton
            label={t.close}
            icon={<X size={24} color={colors.textPrimary} strokeWidth={1.75} />}
            onPress={() => onOpenChange(false)}
          />
        </View>
    ...
    >
      <View className="flex-1 items-center justify-center bg-black/40 p-4">
        <View
          ...
          className="w-full max-w-md gap-4 rounded-lg bg-card p-4"
          style={{ direction: dir }}
        >
