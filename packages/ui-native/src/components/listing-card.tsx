...
export function ListingCard({
  ...
  const photo = card.photo?.url640 ? (
    <Image
      source={{ uri: card.photo.url640 }}
      placeholder={card.photo.blurhash ? { blurhash: card.photo.blurhash } : undefined}
      contentFit="cover"
      transition={150}
  ...
  if (variant === 'map-mini')
    return (
      <Pressable
        ...
        accessibilityLabel={`${name} ${card.year}`}
        onPress={onPress}
        className="flex-row gap-3 rounded-lg border border-border bg-card p-2"
      >
    ...
    return (
      ...
      >
        <View className="aspect-[4/3] w-full bg-brand-subtle">
          {photo}
          {card.featured ? (
            <View className="absolute top-2" style={{ start: 8 }}>
              <Badge kind="featured" />
            </View>
          ) : null}
        </View>
  ...
  return (
    <View className="overflow-hidden rounded-lg border border-border bg-card">
        ...
        <View className="gap-2 p-4">
          ...
          <View className="flex-row items-center gap-1">
            ...
            <Text
              ...
              numberOfLines={1}
              className="shrink"
            >{`${locale === 'ar' ? card.dealer.nameAr : card.dealer.nameEn} · ${card.area[locale]}`}</Text>
      ...
      {favorite ? (
        <View className="absolute top-2" style={{ end: 8 }}>
          {favorite}
        </View>
      ) : null}
      {onContact ? (
...
export function ListingCardSkeleton() {
  ...
  return (
    <View
      ...
      className="overflow-hidden rounded-lg border border-border bg-card opacity-70"
    >
