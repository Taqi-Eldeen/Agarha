...
export function ContactBar({ prices, period = 'day', onContact, contacting, notice }: ContactBarProps) {
  ...
  return (
    <View className="gap-2 border-t border-border bg-card px-3 pt-3 shadow-md" style={{ paddingBottom: Math.max(12, insets.bottom) }}>
      {notice}
      <PriceTag prices={prices} period={period} showDeposit />
      <View className="flex-row gap-2">
        <Button className="flex-1" size="lg" variant="whatsapp" icon={<WhatsAppIcon />} loading={contacting === 'whatsapp'} onPress={() => onContact('whatsapp')}>
          {t.whatsapp}
        </Button>
        <Button size="lg" variant="secondary" icon={<Phone size={20} color={colors.textPrimary} strokeWidth={1.75} />} loading={contacting === 'call'} onPress={() => onContact('call')}>
          {t.call}
        </Button>
      </View>
    </View>
