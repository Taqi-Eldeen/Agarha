...
class GoogleMaps implements MapsProvider {
  ...
  async geocode(address: string, locale: 'ar' | 'en') {
    ...
    const r = (await (await fetch(u, { signal: AbortSignal.timeout(5000) })).json()) as {
      results?: {
  ...
  async geocode(address: string, locale: 'ar' | 'en') {
    ...
    return f
      ? {
          ...
          formatted: f.properties.full_address ?? f.properties.name,
        }
      : null;
  }
}
...
export class MockMaps implements MapsProvider {
  ...
  async geocode(address: string) {
    ...
    return {
      lat: 30.0444 + ((h % 1000) - 500) / 10_000,
      lng: 31.2357 + (((h >> 10) % 1000) - 500) / 10_000,
      formatted: address,
    };
  }
}
...
export class MapsService {
  ...
  constructor(
    ...
    this.provider =
      env.MAPS_PROVIDER === 'google'
        ? new GoogleMaps(env.GOOGLE_MAPS_API_KEY!)
        : env.MAPS_PROVIDER === 'mapbox'
          ? new Mapbox(env.MAPBOX_ACCESS_TOKEN!)
          : new MockMaps();
  }
  ...
  async geocode(address: string, locale: 'ar' | 'en'): Promise<GeocodeResult | null> {
    ...
    try {
      ...
      await this.redis
        .multi()
        .set(key, JSON.stringify(r), 'EX', 30 * 86_400)
        .incr(`maps:spend:${new Date().toISOString().slice(0, 7)}`)
        .exec();
      return r;
    } catch (err) {
  ...
  async monthlyCalls(): Promise<number> {
    return Number(
      (await this.redis.get(`maps:spend:${new Date().toISOString().slice(0, 7)}`)) ?? 0,
    );
  }
}
