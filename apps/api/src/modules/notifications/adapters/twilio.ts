...
export class TwilioSmsProvider implements SmsProvider {
  ...
  async send(to: string, body: string): Promise<SendResult> {
    const form: Record<string, string> = { To: to, MessagingServiceSid: this.messagingServiceSid, Body: body };
    if (this.statusCallback) form.StatusCallback = this.statusCallback;
    const res = (await postForm(this.name, `https://api.twilio.com/2010-04-01/Accounts/${this.accountSid}/Messages.json`, form, {
