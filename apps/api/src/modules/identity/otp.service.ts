...
export class OtpService {
  ...
  async request(input: OtpRequest, ip: string | undefined) {
    ...
    await this.db
      ...
      .set({ codeHash: this.codeHash(challengeId, code) })
    ...
    try {
      ...
      if (err instanceof OtpDeliveryFailed) {
        await this.db
          ...
          .where(eq(otpChallenges.id, challengeId));
        throw new AppError(
          ERROR_CODES.otpDeliveryFailed,
          HttpStatus.BAD_GATEWAY,
          'Could not deliver the code',
        );
      }
      throw err;
    }
  ...
  async verify(
    ...
    const outcome = await this.db.transaction(async (tx) => {
      ...
      if (!c || c.phoneE164 !== phone || !expectedPurposes.includes(c.purpose))
      ...
      if (!safeEqualHex(c.codeHash, this.codeHash(c.id, code))) {
        ...
        return left <= 0
          ...
          : { error: 'invalid' as const, attemptsLeft: left };
      }
      await tx
        .update(otpChallenges)
        .set({ consumedAt: new Date() })
    ...
    if ('error' in outcome) {
      if (outcome.error === 'expired')
        throw new AppError(ERROR_CODES.otpExpired, HttpStatus.BAD_REQUEST, 'Code expired or used');
      if (outcome.error === 'locked')
        throw new AppError(
          ERROR_CODES.otpTooManyAttempts,
          HttpStatus.BAD_REQUEST,
          'Too many attempts',
        );
      throw new AppError(
        ...
        HttpStatus.BAD_REQUEST,
        'Invalid code',
        'attemptsLeft' in outcome ? { attemptsLeft: outcome.attemptsLeft } : undefined,
      );
    }
    return outcome;
  }
