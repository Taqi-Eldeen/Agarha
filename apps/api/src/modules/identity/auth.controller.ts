...
export function readRefresh(req: Request, scope: SessionScope, bodyToken?: string): string {
  const token =
    bodyToken ?? (req.cookies as Record<string, string> | undefined)?.[cookieNames(scope).refresh];
...
export class AuthController {
  ...
  @HttpCode(200)
  @ApiOperation({
    summary: 'Send a sign-in code by SMS (WhatsApp fallback). Requires a Turnstile token.',
  })
  ...
  @HttpCode(200)
  @ApiOperation({
    summary: 'Verify a customer sign-in code. Web gets cookies, mobile gets tokens in the body.',
  })
  ...
  async verify(
    ...
    const { phone } = await this.otp.verify(body.challengeId, body.phone, body.code, client.ip, [
      'customer_sign_in',
    ]);
    ...
    const t = await this.tokens.issue(
      { userId: user.id, scope: 'customer', roles: ['customer'] },
      { userAgent: client.userAgent, ipHash: this.otp.ipHash(client.ip) },
    );
  ...
  @HttpCode(200)
  @ApiOperation({
    summary: 'Verify a dealer sign-up / password-reset code and get a 10-minute phone proof.',
  })
  @ZodBody(otpVerifySchema)
  async proof(
    @Body(new ZodPipe(otpVerifySchema)) body: z.output<typeof otpVerifySchema>,
    ...
    const { phone, purpose } = await this.otp.verify(
      body.challengeId,
      body.phone,
      body.code,
      client.ip,
      ['dealer_sign_up', 'dealer_sign_in'],
    );
  ...
  async refresh(
    @Req() req: Request,
    @Body(new ZodPipe(refreshSchema)) body: z.output<typeof refreshSchema>,
    @Client() client: ClientInfo,
    @Res({ passthrough: true }) res: Response,
  ) {
    ...
    try {
      const t = await this.tokens.rotate(
        presented,
        'customer',
        (userId) => this.users.claimsFor(userId, 'customer'),
      ...
      if (e instanceof AppError && e.code === ERROR_CODES.refreshReused)
  ...
  async signOut(
    ...
    const token =
      body.refreshToken ??
      (req.cookies as Record<string, string> | undefined)?.[cookieNames('customer').refresh];
  ...
  async social(
    @Body(new ZodPipe(socialSchema)) body: z.output<typeof socialSchema>,
    @Client() client: ClientInfo,
    @Res({ passthrough: true }) res: Response,
  ) {
    ...
    if (!identity) {
      ...
      return {
        ...
        linkToken: await this.tokens.signProof('social-link', {
    ...
    return {
      status: 'signed_in',
      user: this.users.toSessionUser(user, ['customer']),
      ...(tokens ? { tokens } : {}),
    };
  }
  ...
  async socialLink(
    @Body(new ZodPipe(socialLinkSchema)) body: z.output<typeof socialLinkSchema>,
    @Client() client: ClientInfo,
    @Res({ passthrough: true }) res: Response,
  ) {
    const link = await this.tokens.verifyProof<{
      provider: 'google' | 'apple';
      subject: string;
      email: string | null;
    }>('social-link', body.linkToken);
    const { phone } = await this.otp.verify(body.challengeId, body.phone, body.code, client.ip, [
      'customer_sign_in',
    ]);
    const user = await this.users.findOrCreateByPhone(phone, 'ar');
  ...
  async updateMe(
    ...
    @Body(new ZodPipe(updateMeSchema)) body: z.output<typeof updateMeSchema>,
  ) {
  ...
  @Auth('customer')
  @ApiOperation({
    summary: 'PDPL data export: everything we hold about the signed-in user, as JSON.',
  })
  ...
  @Auth('customer')
  @ApiOperation({
    summary: 'Delete account: anonymises the user immediately and revokes every session.',
  })
