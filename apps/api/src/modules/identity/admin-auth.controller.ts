...
export class AdminAuthController {
  ...
  async google(@Body(new ZodPipe(googleSchema)) body: z.output<typeof googleSchema>) {
    ...
    if (!id.emailVerified || id.hostedDomain !== this.env.GOOGLE_WORKSPACE_DOMAIN)
      throw Errors.forbidden('Use your company Google account');
  ...
  private async afterSso(subject: string, email?: string) {
    ...
    if (!cred?.totpEnabledAt) {
      ...
      return {
        ...
        setupToken: await this.tokens.signProof(
          'admin-totp-setup',
          {
            userId: identity.userId,
            secretEnc: encryptSecret(secret, this.env.TOTP_ENCRYPTION_KEY),
          },
          900,
        ),
    ...
    return {
      next: 'totp' as const,
      loginToken: await this.tokens.signProof('admin-login', { userId: identity.userId }, 300),
    };
  }
  ...
  @ZodBody(setupSchema)
  async setup(
    @Body(new ZodPipe(setupSchema)) body: z.output<typeof setupSchema>,
  ...
  @ZodBody(codeSchema)
  async totp(
    @Body(new ZodPipe(codeSchema)) body: z.output<typeof codeSchema>,
    @Client() info: ClientInfo,
    @Res({ passthrough: true }) res: Response,
  ) {
  ...
  async refresh(
    ...
    const t = await this.tokens.rotate(
      readRefresh(req, 'admin'),
      'admin',
      (userId) => this.users.claimsFor(userId, 'admin'),
