...
export class DealerAuthController {
  ...
  async register(@Body(new ZodPipe(registerSchema)) body: z.output<typeof registerSchema>) {
    const proof = await this.tokens.verifyProof<{ phone: string; purpose: string }>(
      'phone',
      body.phoneProof,
    );
    ...
    if (existing?.passwordHash)
      throw Errors.conflict('This number already has a dealer login. Sign in instead.');
    await this.users.upsertCredentials(user.id, {
  ...
  private async startTotpSetup(userId: string, phone: string) {
    ...
    const setupToken = await this.tokens.signProof(
      'totp-setup',
      { userId, secretEnc: encryptSecret(secret, this.env.TOTP_ENCRYPTION_KEY) },
      900,
    );
    return {
      next: 'totp_setup' as const,
      setupToken,
      totp: { secret, uri: totpUri(secret, phone) },
    };
  }
  ...
  async confirmTotp(
    @Body(new ZodPipe(totpConfirmSchema)) body: z.output<typeof totpConfirmSchema>,
    @Client() info: ClientInfo,
    @Res({ passthrough: true }) res: Response,
  ) {
    const p = await this.tokens.verifyProof<{ userId: string; secretEnc: string }>(
      'totp-setup',
      body.setupToken,
    );
    const step = verifyTotp(
      decryptSecret(p.secretEnc, this.env.TOTP_ENCRYPTION_KEY),
      body.code,
      null,
    );
    if (step === null)
      throw new AppError(
        ERROR_CODES.otpInvalid,
        HttpStatus.BAD_REQUEST,
        'Invalid authenticator code',
      );
    await this.users.upsertCredentials(p.userId, {
      totpSecretEnc: p.secretEnc,
  ...
  async login(
    ...
    await this.limiter.hit(
      { name: 'dealer-login:phone', key: body.phone, max: 20, windowSeconds: 900 },
      {
        name: 'dealer-login:ip',
        key: this.otp.ipHash(info.ip) ?? 'x',
        max: 30,
        windowSeconds: 900,
      },
    );
    ...
    if (!user || !cred?.passwordHash || user.status !== 'active')
      throw Errors.unauthorized('Wrong phone number or password');
    if (cred.lockedUntil && cred.lockedUntil.getTime() > Date.now())
      throw Errors.rateLimited(Math.ceil((cred.lockedUntil.getTime() - Date.now()) / 1000));
    if (!ok) {
      const failed = cred.failedPasswordAttempts + 1;
      await this.users.upsertCredentials(user.id, {
        failedPasswordAttempts: failed,
        ...(failed >= LOCK_AFTER
          ? { lockedUntil: new Date(Date.now() + LOCK_MINUTES * 60_000), failedPasswordAttempts: 0 }
          : {}),
      });
      throw Errors.unauthorized('Wrong phone number or password');
    }
    ...
    if (cred.totpEnabledAt)
      return {
        next: 'totp' as const,
        loginToken: await this.tokens.signProof(
          'dealer-login',
          { userId: user.id, factor: 'totp' },
          300,
        ),
      };
    ...
    const sent = await this.otp.request(
      {
        phone: body.phone,
    ...
    return {
      ...
      loginToken: await this.tokens.signProof(
        'dealer-login',
        { userId: user.id, factor: 'otp', phone: body.phone },
        600,
      ),
    };
  }
  ...
  async loginVerify(
    @Body(new ZodPipe(loginVerifySchema)) body: z.output<typeof loginVerifySchema>,
    @Client() info: ClientInfo,
    @Res({ passthrough: true }) res: Response,
  ) {
    const p = await this.tokens.verifyProof<{
      userId: string;
      factor: 'totp' | 'otp';
      phone?: string;
    }>('dealer-login', body.loginToken);
    if (p.factor === 'totp') {
      ...
      const step = verifyTotp(
        decryptSecret(cred.totpSecretEnc, this.env.TOTP_ENCRYPTION_KEY),
        body.code,
        cred.totpLastStep,
      );
      if (step === null)
      ...
      if (!body.challengeId || !p.phone)
        throw Errors.badRequest(ERROR_CODES.validation, 'challengeId required');
  ...
  async refresh(
    @Req() req: Request,
    @Body(new ZodPipe(refreshSchema.extend({ dealerId: z.uuid().optional() })))
    body: { refreshToken?: string; client: 'web' | 'mobile'; dealerId?: string },
    @Client() info: ClientInfo,
    @Res({ passthrough: true }) res: Response,
  ) {
    const presented = readRefresh(req, 'dealer', body.refreshToken);
    let claims: Claims | null = null;
    const t = await this.tokens.rotate(
      presented,
      'dealer',
      async (userId, dealerId) =>
        (claims = await this.users.claimsFor(userId, 'dealer', body.dealerId ?? dealerId)),
      { userAgent: info.userAgent },
    );
    const tokens = deliverTokens(
      res,
      this.env,
      'dealer',
      body.refreshToken ? 'mobile' : body.client,
      t,
    );
  ...
  private async session(
    ...
    const t = await this.tokens.issue(claims, {
      userAgent: info.userAgent,
      ipHash: this.otp.ipHash(info.ip),
    });
