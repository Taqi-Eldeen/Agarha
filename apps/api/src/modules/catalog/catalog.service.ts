...
export class CatalogService {
  ...
  models(makeId: string) {
    return this.db
      .select({
        id: carModels.id,
        makeId: carModels.makeId,
        slug: carModels.slug,
        nameAr: carModels.nameAr,
        nameEn: carModels.nameEn,
  ...
  trims(modelId: string) {
    return this.db
      .select()
      .from(carTrims)
      .where(eq(carTrims.modelId, modelId))
      .orderBy(asc(carTrims.nameEn));
  }
  ...
  async upsertCity(
    ...
    const values = {
      ...
      center: v.lat != null && v.lng != null ? point(v.lng, v.lat) : null,
    };
    if (v.id)
      return (
        await this.db
  ...
  async upsertArea(
    v: { id?: string; cityId: string; slug: string; nameAr: string; nameEn: string } & GeoInput,
  ) {
    const values = {
      cityId: v.cityId,
      slug: v.slug,
  ...
  async upsertModel(v: {
    ...
    nameAr: string;
    nameEn: string;
    bodyType: typeof carModels.$inferInsert.bodyType;
  }) {
    const { id, ...values } = v;
    if (id)
      return (
    ...
    if (id)
      return (
        await this.db
          .update(carTrims)
          .set(values)
          .where(eq(carTrims.id, id))
          .returning({ id: carTrims.id })
      )[0];
