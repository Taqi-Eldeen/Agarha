...
export class ModerationService implements OnModuleInit {
  ...
  async report(
    userId: string,
    input: { listingId: string; reason: ReportReason; details?: string | undefined },
  ) {
    ...
    const [dup] = await this.db
      ...
      .where(
        and(
          ...
          eq(reports.listingId, input.listingId),
  ...
  queue(status: 'open' | 'actioned' | 'dismissed' = 'open') {
    ...
    return this.db
      ...
      .orderBy(desc(eq(reports.reason, 'scam_or_deposit_request')), reports.createdAt)
      .limit(200);
  }
  ...
  async resolve(
    ...
    if (action === 'hide_listing' && r.listingId)
      await this.listings.hideForReport(r.listingId, note, actor);
  ...
  async countsForDealer(dealerId: string) {
    return this.db
      .select()
      .from(reports)
      .where(eq(reports.dealerId, dealerId))
