...
export class ListingsService implements OnModuleInit {
  ...
  onModuleInit() {
    ...
    this.events.on('dealer.suspended', async ({ dealerId }) => {
      const r = await this.db
        .update(listings)
        .set({ status: 'hidden', hiddenReason: 'dealer_suspended' })
        .where(and(eq(listings.dealerId, dealerId), eq(listings.status, 'live')))
    ...
    this.events.on('dealer.unsuspended', async ({ dealerId }) => {
      const r = await this.db
        .update(listings)
        .set({ status: 'live', hiddenReason: null })
        .where(
          and(
            eq(listings.dealerId, dealerId),
            eq(listings.status, 'hidden'),
            eq(listings.hiddenReason, 'dealer_suspended'),
          ),
        )
        .returning({ id: listings.id });
      await this.changed(...r.map((x) => x.id));
    ...
    this.events.on('featured.activated', async ({ listingId, endsAt }) => {
      await this.db
        .update(listings)
        .set({ featuredUntil: new Date(endsAt) })
        ...
        ? Object.fromEntries(
            PHOTO_FORMATS.map((f) => [
              ...
              Object.fromEntries(
                PHOTO_WIDTHS.map((w) => [
                  w,
                  this.storage.publicUrl(variants[f][String(w) as '320']),
                ]),
              ),
            ]),
          )
  ...
  async load(ids: string[]) {
    ...
    const rows = await this.db
      .select({ l: listings, p: listingPrices })
      .from(listings)
      .innerJoin(listingPrices, eq(listingPrices.listingId, listings.id))
      .where(inArray(listings.id, ids));
    const photos = await this.db
      .select()
      .from(listingPhotos)
      .where(and(inArray(listingPhotos.listingId, ids), eq(listingPhotos.status, 'ready')))
  ...
  dto(l: ListingRow, p: PriceRow, photos: PhotoRow[]) {
    ...
    return {
      ...
      prices: {
        day: p.priceDayEgp,
        week: p.priceWeekEgp,
        month: p.priceMonthEgp,
        deposit: p.depositEgp,
      },
  ...
  private async owned(dealerId: string, id: string) {
    const [l] = await this.db
      .select()
      .from(listings)
      .where(and(eq(listings.id, id), eq(listings.dealerId, dealerId)));
  ...
  async fleet(
    dealerId: string,
    q: { status?: string | undefined; cursor?: string | undefined; limit: number },
  ) {
    ...
    const rows = await this.db
      ...
      .where(
        and(
          eq(listings.dealerId, dealerId),
          q.status ? eq(listings.status, q.status as never) : ne(listings.status, 'archived'),
          c
            ? or(
                lt(listings.updatedAt, new Date(c.t)),
                and(eq(listings.updatedAt, new Date(c.t)), lt(listings.id, c.id)),
              )
            : undefined,
        ),
      )
    ...
    for (const i of items)
      if (!models.has(i.carModelId))
        models.set(i.carModelId, await this.catalog.model(i.carModelId));
  ...
  async create(dealerId: string, input: CreateListing, actor: Actor) {
    ...
    const id = await this.db.transaction(async (tx) => {
      ...
      await tx.insert(listingPrices).values({
        listingId: l!.id,
        priceDayEgp: input.priceDayEgp,
        priceWeekEgp: input.priceWeekEgp ?? null,
        priceMonthEgp: input.priceMonthEgp ?? null,
        depositEgp: input.depositEgp,
      });
  ...
  async update(dealerId: string, id: string, input: CreateListing, actor: Actor) {
    ...
    await this.db.transaction(async (tx) => {
      ...
      await tx
        ...
        .where(eq(listingPrices.listingId, id));
      await this.audit.record(
        {
          actorUserId: actor.userId,
          actorRole: actor.role,
  ...
  async publish(dealerId: string, id: string, actor: Actor) {
    const l = await this.owned(dealerId, id);
    if (!['draft', 'paused', 'hidden'].includes(l.status))
      throw Errors.conflict(`Cannot publish a ${l.status} listing`);
    if (l.status === 'hidden' && l.hiddenReason !== 'stale')
      throw Errors.conflict('This listing was hidden by moderation');
    const d = await this.dealers.get(dealerId);
    if (!this.dealers.isPublic(d))
      throw Errors.conflict('Your business must be verified before cars go live');
    const [{ n: photos } = { n: 0 }] = await this.db
      .select({ n: sql<number>`count(*)::int` })
      .from(listingPhotos)
      .where(and(eq(listingPhotos.listingId, id), eq(listingPhotos.status, 'ready')));
    ...
    if (limits.maxLiveListings !== null) {
      ...
      if (live >= limits.maxLiveListings)
        throw Errors.conflict('Your plan does not allow more live cars', {
          maxLiveListings: limits.maxLiveListings,
        });
    }
    const [{ n: approved } = { n: 0 }] = await this.db
      .select({ n: sql<number>`count(*)::int` })
      .from(listings)
      .where(and(eq(listings.dealerId, dealerId), sql`${listings.reviewedAt} IS NOT NULL`));
    ...
    await this.db
      ...
      .set({
        ...
        hiddenReason: null,
        lastConfirmedAt: new Date(),
        ...(status === 'live' && !l.publishedAt ? { publishedAt: new Date() } : {}),
      })
      .where(eq(listings.id, id));
    await this.audit.record({
      actorUserId: actor.userId,
      actorRole: actor.role,
      action: status === 'live' ? 'listing.publish' : 'listing.submit',
      targetType: 'listing',
      targetId: id,
      dealerId,
    });
    if (status === 'live')
      await this.events.publish('listing.published', { listingId: id, dealerId });
    await this.changed(id);
    return this.one(id);
  }
  ...
  async setStatus(dealerId: string, id: string, status: 'paused' | 'archived', actor: Actor) {
    ...
    await this.audit.record({
      actorUserId: actor.userId,
      actorRole: actor.role,
      action: `listing.${status === 'paused' ? 'pause' : 'archive'}`,
      targetType: 'listing',
      targetId: id,
      dealerId,
    });
  ...
  async setAvailability(dealerId: string, id: string, available: boolean, actor: Actor) {
    ...
    const [r] = await this.db
      .update(listings)
      .set({ available, lastConfirmedAt: new Date() })
      .where(eq(listings.id, id))
      .returning({ available: listings.available, lastConfirmedAt: listings.lastConfirmedAt });
    await this.audit.record({
      actorUserId: actor.userId,
      actorRole: actor.role,
    ...
    return {
      ...
      freshness: freshnessOf(r!.lastConfirmedAt),
    };
  }
  ...
  async confirmAll(dealerId: string, actor: Actor) {
    ...
    const confirmed = await this.db
      ...
      .where(
        and(
          eq(listings.dealerId, dealerId),
          inArray(listings.status, ['live', 'paused', 'pending']),
        ),
      )
      .returning({ id: listings.id });
    const restored = await this.db
      .update(listings)
      .set({ status: 'live', hiddenReason: null, lastConfirmedAt: now })
      .where(
        and(
          eq(listings.dealerId, dealerId),
          eq(listings.status, 'hidden'),
          eq(listings.hiddenReason, 'stale'),
        ),
      )
      .returning({ id: listings.id });
    await this.audit.record({
      actorUserId: actor.userId,
      actorRole: actor.role,
      action: 'listing.confirm_all',
      targetType: 'dealer',
      targetId: dealerId,
      dealerId,
      metadata: { confirmed: confirmed.length, restored: restored.length },
    });
    await this.changed(...confirmed.map((r) => r.id), ...restored.map((r) => r.id));
    return {
      confirmed: confirmed.length + restored.length,
  ...
  async branchHasListings(branchId: string) {
    const [r] = await this.db
      .select({ n: sql<number>`count(*)::int` })
      .from(listings)
      .where(and(eq(listings.branchId, branchId), ne(listings.status, 'archived')));
    return (r?.n ?? 0) > 0;
  }
  ...
  async createPhotoUpload(
    ...
    const existing = await this.db
      ...
      .from(listingPhotos)
      .where(and(eq(listingPhotos.listingId, listingId), ne(listingPhotos.status, 'rejected')));
    if (existing.length >= MAX_PHOTOS_PER_LISTING)
      throw Errors.conflict(`A car can have at most ${MAX_PHOTOS_PER_LISTING} photos`);
    ...
    await this.db
      ...
      .where(
        and(
          ...
          eq(listingPhotos.position, position),
          eq(listingPhotos.status, 'rejected'),
        ),
      );
    ...
    await this.db
      ...
      .values({ id: photoId, listingId, position, status: 'pending_upload', storageKey: key });
    // Uploads land in the private bucket; only processed, EXIF-free variants are public.
    return {
      photoId,
      position,
      upload: await this.storage.presignPut('private', key, input.mimeType, input.sizeBytes),
    };
  }
    ...
    const r = await this.db
      ...
      .where(
        and(
          ...
          eq(listingPhotos.listingId, listingId),
          eq(listingPhotos.status, 'pending_upload'),
        ),
      )
  ...
  async processPhoto(photoId: string) {
    ...
    try {
      ...
      const variants = { webp: {}, avif: {} } as Record<
        'webp' | 'avif',
        Record<'320' | '640' | '1280', string>
      >;
  ...
  async reorderPhotos(dealerId: string, listingId: string, photoIds: string[]) {
    ...
    if (photoIds.some((id) => !known.has(id)))
      throw Errors.badRequest('validation_failed', 'Unknown photo');
    await this.db.transaction(async (tx) => {
      ...
      for (const [i, id] of photoIds.entries())
        await tx.update(listingPhotos).set({ position: i }).where(eq(listingPhotos.id, id));
    });
    await this.changed(listingId);
    ...
    if (p.variants)
      for (const f of PHOTO_FORMATS)
        for (const w of PHOTO_WIDTHS)
          await this.storage
            .delete('public', p.variants[f][String(w) as '320'])
            .catch(() => undefined);
  }
  ...
  async adminQueue(q: {
    ...
    const filter =
      q.queue === 'pending'
        ...
        : q.queue === 'unreviewed'
          ? and(eq(listings.status, 'live'), isNull(listings.reviewedAt))
          : q.queue === 'hidden'
            ? eq(listings.status, 'hidden')
            : undefined;
    const c = decodeCursor<{ t: string; id: string }>(q.cursor);
    const rows = await this.db
      ...
      .where(
        and(
          filter,
          q.dealerId ? eq(listings.dealerId, q.dealerId) : undefined,
          c
            ? or(
                gt(listings.updatedAt, new Date(c.t)),
                and(eq(listings.updatedAt, new Date(c.t)), gt(listings.id, c.id)),
              )
            : undefined,
        ),
      )
    ...
    const last = page.at(-1);
    return {
      items: await this.load(page.map((r) => r.id)),
      nextCursor:
        rows.length > q.limit && last
          ? encodeCursor({ t: last.updatedAt.toISOString(), id: last.id })
          : null,
  ...
  async moderate(
    id: string,
    d: { decision: 'approve' } | { decision: 'reject' | 'hide'; reason: string },
    actor: Actor,
  ) {
    ...
    if (d.decision === 'approve') {
      ...
      await this.db
        ...
        .where(eq(listings.id, id));
      if (goLive)
        await this.events.publish('listing.published', { listingId: id, dealerId: l.dealerId });
    } else {
      await this.db
        .update(listings)
        .set({
          status: d.decision === 'reject' ? 'draft' : 'hidden',
          hiddenReason: `moderation: ${d.reason}`,
          reviewedAt: new Date(),
        })
        .where(eq(listings.id, id));
      await this.events.publish('listing.hidden', {
        listingId: id,
        dealerId: l.dealerId,
        reason: d.reason,
      });
    ...
    await this.audit.record({
      actorUserId: actor.userId,
      actorRole: actor.role,
      action: `listing.moderate.${d.decision}`,
      targetType: 'listing',
      targetId: id,
      dealerId: l.dealerId,
      ...('reason' in d ? { metadata: { reason: d.reason } } : {}),
    });
    await this.changed(id);
    return this.one(id);
  }
  ...
  async publicOne(id: string) {
    const [l] = await this.db
      .select({ id: listings.id, status: listings.status, dealerId: listings.dealerId })
      .from(listings)
      .where(eq(listings.id, id));
  ...
  async liveIds() {
    return (
      await this.db.select({ id: listings.id }).from(listings).where(eq(listings.status, 'live'))
    ).map((r) => r.id);
  }
  ...
  async freshnessStats() {
    ...
    return {
      live: r?.live ?? 0,
      confirmedWithin7Days: r?.fresh7 ?? 0,
      freshPercent: r && r.live ? Math.round((r.fresh7 / r.live) * 1000) / 10 : null,
      pending: r?.pending ?? 0,
      staleHidden: r?.staleHidden ?? 0,
    };
  }
  ...
  async needsConfirmation(olderThanMs: number) {
    ...
    return this.db
      .select({ dealerId: listings.dealerId, n: sql<number>`count(*)::int` })
      .from(listings)
      .where(
        and(
          inArray(listings.status, ['live', 'hidden']),
          lt(listings.lastConfirmedAt, cutoff),
          or(ne(listings.status, 'hidden'), eq(listings.hiddenReason, 'stale')),
        ),
      )
  ...
  async runFreshness(now = new Date()) {
    ...
    const hidden = await this.db
      .update(listings)
      .set({ status: 'hidden', hiddenReason: 'stale' })
      .where(and(eq(listings.status, 'live'), lt(listings.lastConfirmedAt, cutoff)))
      .returning({
        id: listings.id,
        dealerId: listings.dealerId,
        carModelId: listings.carModelId,
        year: listings.year,
      });
    for (const h of hidden) {
      ...
      await this.notifications.notify({
        template: 'listing_hidden',
        locale: 'ar',
        vars: { car: `${model?.makeNameAr ?? ''} ${model?.nameAr ?? ''} ${h.year}`.trim() },
        channels: ['whatsapp'],
        whatsappTo: contacts.whatsapp,
        related: { type: 'listing', id: h.id },
      });
      await this.events.publish('listing.hidden', {
        listingId: h.id,
        dealerId: h.dealerId,
  ...
  async runNudges() {
    ...
    for (const r of rows) {
      ...
      await this.notifications.notify({
        template: 'availability_nudge',
        locale: 'ar',
        vars: { count: r.n },
        channels: ['whatsapp'],
        whatsappTo: c.whatsapp,
        related: { type: 'dealer', id: r.dealerId },
      });
    }
