import { DRIVER_OPTIONS, FUELS, REQUIRED_DOCS, TRANSMISSIONS, normalizeArabic } from '@agarha/schemas';
...
export const CSV_COLUMNS = ['make', 'model', 'year', 'color', 'transmission', 'fuel', 'seats', 'driver_option', 'price_day', 'price_week', 'price_month', 'deposit', 'min_age', 'required_docs', 'km_limit_per_day', 'airport_pickup', 'branch'] as const;
const MAX_ROWS = 200;
...
export class ImportService {
  ...
  private async resolveModel(make: string, model: string) {
    ...
    const mk = makes.find((m) => normalizeArabic(m.nameEn) === nm || normalizeArabic(m.nameAr) === nm || m.slug === nm);
    if (!mk) return null;
    const models = await this.catalog.models(mk.id);
    const nmod = normalizeArabic(model);
    return models.find((m) => normalizeArabic(m.nameEn) === nmod || normalizeArabic(m.nameAr) === nmod || m.slug === nmod) ?? null;
  }

  async validate(dealerId: string, csv: string, actor: Actor) {
    let rows: Row[];
    try {
      rows = parse(csv, { columns: (h: string[]) => h.map((c) => c.trim().toLowerCase()), skip_empty_lines: true, trim: true, bom: true }) as Row[];
    } catch {
    ...
    if (rows.length > MAX_ROWS) throw Errors.badRequest('validation_failed', `At most ${MAX_ROWS} cars per file`);
    ...
    for (const [i, r] of rows.entries()) {
      ...
      const branch = r.branch ? branches.find((b) => normalizeArabic(b.nameEn) === normalizeArabic(r.branch) || normalizeArabic(b.nameAr) === normalizeArabic(r.branch)) : branches.find((b) => b.isPrimary);
      if (!branch) {
      ...
      const candidate = {
        ...
        requiredDocs: (r.required_docs ?? '').split(/[|;]/).map((d) => d.trim()).filter(Boolean),
      ...
      if (!parsed.success) for (const issue of parsed.error.issues) errors.push({ row: rowNo, field: String(issue.path[0] ?? 'row'), code: issue.message });
      else valid.push(parsed.data);
    }
    const [imp] = await this.db.insert(listingImports).values({ dealerId, createdBy: actor.userId, status: errors.length ? 'failed' : 'ready', rowCount: rows.length, errors, rows: errors.length ? null : valid }).returning();
    return { importId: imp!.id, status: imp!.status, rowCount: rows.length, errors, allowed: { transmission: TRANSMISSIONS, fuel: FUELS, driver_option: DRIVER_OPTIONS, required_docs: REQUIRED_DOCS } };
  }

  async apply(dealerId: string, importId: string, actor: Actor) {
    const [imp] = await this.db.select().from(listingImports).where(and(eq(listingImports.id, importId), eq(listingImports.dealerId, dealerId)));
    if (!imp) throw Errors.notFound('Import');
    if (imp.status !== 'ready') throw Errors.conflict('This import cannot be applied');
    const rows = imp.rows ?? [];
    const created: string[] = [];
    for (const r of rows) created.push((await this.listings.create(dealerId, r as never, actor)).id);
    await this.db.update(listingImports).set({ status: 'applied', appliedAt: new Date(), rows: null }).where(eq(listingImports.id, importId));
