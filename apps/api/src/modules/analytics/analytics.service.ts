...
export class AnalyticsService implements OnModuleInit {
  ...
  onModuleInit() {
    this.events.on('lead.created', async (e) => {
      await this.bump(
        e.listingId,
        e.dealerId,
        e.channel === 'whatsapp' ? 'whatsappContacts' : 'callContacts',
      );
      this.flags.capture({
        event: 'lead_created',
        distinctId: e.userId ?? `anon:${e.leadId}`,
        properties: {
          channel: e.channel,
  ...
  private async bump(
    listingId: string,
    dealerId: string,
    col: 'views' | 'whatsappContacts' | 'callContacts',
  ) {
    ...
    await this.db
      ...
      .onConflictDoUpdate({
        target: [listingDailyStats.listingId, listingDailyStats.day],
        set: { [col]: sql`${column} + 1` },
      });
  }
  ...
  async recordView(listingId: string, dealerId: string, visitor: string) {
    ...
    if (await this.redis.set(key, '1', 'EX', 90_000, 'NX'))
      await this.bump(listingId, dealerId, 'views');
  }
  ...
  async dealerStats(dealerId: string, days: number) {
    ...
    const rows = await this.db
      .select({
        ...
        views: sql<number>`sum(${listingDailyStats.views})::int`,
        whatsapp: sql<number>`sum(${listingDailyStats.whatsappContacts})::int`,
        calls: sql<number>`sum(${listingDailyStats.callContacts})::int`,
      })
      .from(listingDailyStats)
      .where(
        and(eq(listingDailyStats.dealerId, dealerId), gte(listingDailyStats.day, cairoDay(since))),
      )
      .groupBy(listingDailyStats.listingId);
    const daily = await this.db
      .select({
        day: listingDailyStats.day,
        views: sql<number>`sum(${listingDailyStats.views})::int`,
        contacts: sql<number>`sum(${listingDailyStats.whatsappContacts} + ${listingDailyStats.callContacts})::int`,
      })
      .from(listingDailyStats)
      .where(
        and(eq(listingDailyStats.dealerId, dealerId), gte(listingDailyStats.day, cairoDay(since))),
    ...
    const totals = cars.reduce(
      (t, c) => ({
        views: t.views + c.views,
        whatsapp: t.whatsapp + c.whatsapp,
    ...
    return {
      ...
      totals: {
        ...totals,
        conversion: totals.views
          ? Math.round(((totals.whatsapp + totals.calls) / totals.views) * 1000) / 10
          : null,
      },
      freshnessScore: live.length
        ? Math.round(live.reduce((s, c) => s + c.freshnessScore, 0) / live.length)
        : null,
      daily,
      cars,
    };
  ...
  async viewsByListing(ids: string[], since: Date) {
    ...
    return this.db
      ...
      .from(listingDailyStats)
      .where(
        and(inArray(listingDailyStats.listingId, ids), gte(listingDailyStats.day, cairoDay(since))),
      )
      .groupBy(listingDailyStats.listingId);
  }
  ...
  async platformDaily(days: number) {
    return this.db
      .select({
        day: listingDailyStats.day,
        views: sql<number>`sum(${listingDailyStats.views})::int`,
        leads: sql<number>`sum(${listingDailyStats.whatsappContacts} + ${listingDailyStats.callContacts})::int`,
      })
      .from(listingDailyStats)
