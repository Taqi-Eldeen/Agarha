...
export class ReviewsService implements OnModuleInit {
  ...
  async create(
    userId: string,
    input: { leadId: string; rating: number; body?: string | undefined },
  ) {
    ...
    if (!lead)
      throw Errors.forbidden('You can review a dealer only after contacting them through Agarha');
    if (!lead.oldEnough)
      throw Errors.conflict('You can write a review 24 hours after contacting the dealer');
    const [existing] = await this.db
      .select({ id: reviews.id })
      .from(reviews)
      .where(eq(reviews.leadId, lead.id));
    ...
    const [recent] = await this.db
      ...
      .where(
        and(
          ...
          eq(reviews.dealerId, lead.dealerId),
  ...
  async publicForDealer(dealerId: string, q: { cursor?: string | undefined; limit: number }) {
    ...
    const rows = await this.db
      .select({
        ...
        rating: reviews.rating,
        body: reviews.body,
        dealerReply: reviews.dealerReply,
        dealerRepliedAt: reviews.dealerRepliedAt,
        createdAt: reviews.createdAt,
      })
      .from(reviews)
      .where(
        and(
          eq(reviews.dealerId, dealerId),
          eq(reviews.status, 'published'),
          c
            ? or(
                lt(reviews.createdAt, new Date(c.t)),
                and(eq(reviews.createdAt, new Date(c.t)), lt(reviews.id, c.id)),
              )
            : undefined,
        ),
      )
  ...
  async summary(dealerId: string) {
    const [s] = await this.db
      .select({
        ...
        avg: sql<number | null>`round(avg(${reviews.rating})::numeric, 1)::float`,
      })
      .from(reviews)
      .where(and(eq(reviews.dealerId, dealerId), eq(reviews.status, 'published')));
  ...
  async reply(
    ...
    const r = await this.db
      .update(reviews)
      .set({ dealerReply: text, dealerRepliedAt: new Date() })
      .where(
        and(eq(reviews.id, id), eq(reviews.dealerId, dealerId), eq(reviews.status, 'published')),
      )
      .returning();
  ...
  async moderate(
    id: string,
    approve: boolean,
    actor: { userId: string; role: 'admin' | 'moderator' },
    reason?: string,
  ) {
    const [r] = await this.db
      .update(reviews)
    ...
    if (approve)
      await this.events.publish('review.published', { reviewId: id, dealerId: r.dealerId });
    return r;
  }
}
