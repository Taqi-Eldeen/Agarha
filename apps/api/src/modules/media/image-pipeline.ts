...
export interface ProcessedImage {
  ...
  variants: {
    ...
    width: (typeof PHOTO_WIDTHS)[number];
    body: Buffer;
  }[];
}
...
export async function processListingPhoto(input: Buffer): Promise<ProcessedImage> {
  ...
  for (const width of PHOTO_WIDTHS) {
    ...
    variants.push({
      format: 'avif',
      width,
      body: await resized.clone().avif({ quality: 55, effort: 4 }).toBuffer(),
    });
  }
  const oriented = await sharp(original).metadata();
  return {
    width: oriented.width ?? meta.width,
    height: oriented.height ?? meta.height,
    blurhash,
    original,
    variants,
  };
...
export async function sanitiseDocument(
  input: Buffer,
): Promise<{ body: Buffer; type: 'application/pdf' | 'image/jpeg' }> {
  if (input.length > MAX_PHOTO_BYTES) throw new RejectedMedia('too_large');
  const type = sniff(input);
  if (type === 'application/pdf') return { body: input, type };
  if (type === 'image/jpeg' || type === 'image/png') {
    return {
      body: await sharp(input, { failOn: 'error' }).rotate().jpeg({ quality: 90 }).toBuffer(),
      type: 'image/jpeg',
    };
  }
