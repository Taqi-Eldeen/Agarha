...
export const REQUIRED_DOC_TYPES: VerificationDocType[] = [
...
export class VerificationService {
  ...
  async list(dealerId: string) {
    return this.db
      .select({
        id: verificationDocs.id,
        type: verificationDocs.type,
        status: verificationDocs.status,
        mimeType: verificationDocs.mimeType,
        sizeBytes: verificationDocs.sizeBytes,
        rejectionReason: verificationDocs.rejectionReason,
        createdAt: verificationDocs.createdAt,
  ...
  async checklist(dealerId: string) {
    ...
    const items = REQUIRED_DOC_TYPES.map((type) => ({
      type,
      status: latest.get(type)?.status ?? 'missing',
      rejectionReason: latest.get(type)?.rejectionReason ?? null,
    }));
    return {
      items,
      requiredUploaded: items.every((i) => i.status !== 'missing' && i.status !== 'rejected'),
    };
  }
  ...
  async createUpload(
    ...
    if (input.sizeBytes > MAX_PHOTO_BYTES)
      throw Errors.badRequest('validation_failed', 'File is larger than 10 MB');
    ...
    await this.audit.record({
      ...
      action: 'verification_doc.upload',
  ...
  async complete(dealerId: string, docId: string) {
    const [d] = await this.db
      .select()
      .from(verificationDocs)
      .where(and(eq(verificationDocs.id, docId), eq(verificationDocs.dealerId, dealerId)));
  ...
  async process(docId: string) {
    ...
    try {
      ...
      await this.db
        ...
        .set({ mimeType: clean.type, sizeBytes: clean.body.length })
      ...
      await this.db
        ...
        .set({
          ...
          deleteAfter: new Date(Date.now() + REJECTED_RETENTION_DAYS * 86_400_000),
        })
        .where(eq(verificationDocs.id, docId));
    }
  }
  ...
  async review(
    ...
    await this.audit.record({
      actorUserId: actor.userId,
      actorRole: actor.role,
      action: decision.approve ? 'verification_doc.approve' : 'verification_doc.reject',
      targetType: 'verification_doc',
      targetId: docId,
      dealerId: d.dealerId,
      ...(decision.approve ? {} : { metadata: { reason: decision.reason } }),
    });
  ...
  async purgeExpired(now = new Date()) {
    const expired = await this.db
      .select({ id: verificationDocs.id, key: verificationDocs.storageKey })
      .from(verificationDocs)
      .where(and(eq(verificationDocs.status, 'rejected'), lt(verificationDocs.deleteAfter, now)));
