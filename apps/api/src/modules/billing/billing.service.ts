...
export class BillingService {
  ...
  listPlans(includeInactive = false) {
    return this.db
      .select()
      .from(plans)
      .where(includeInactive ? undefined : eq(plans.isActive, true))
  ...
  async limits(dealerId: string): Promise<Limits> {
    ...
    return {
      planCode: code,
      maxLiveListings: p?.maxLiveListings ?? null,
      maxTeamMembers: p?.maxTeamMembers ?? 3,
      featuredCreditsRemaining: s?.featuredCreditsRemaining ?? 0,
    };
  }
  ...
  async overview(dealerId: string) {
    ...
    const featured = await this.db
      ...
      .where(
        and(eq(featuredPlacements.dealerId, dealerId), gt(featuredPlacements.endsAt, new Date())),
      )
      .orderBy(asc(featuredPlacements.endsAt));
    return {
      subscription: await this.subscription(dealerId),
      limits: await this.limits(dealerId),
      invoices: invs,
      activeFeatured: featured,
      featuredPricePerDayEgp: this.env.FEATURED_PRICE_PER_DAY_EGP,
    };
  }

  private async nextInvoiceNumber(): Promise<string> {
    const r = await this.db.execute<{ n: string }>(
      sql`SELECT nextval('invoice_number_seq')::text AS n`,
    );
  ...
  async subscribe(
    ...
    const inv = await this.openInvoice(dealerId, [
      {
        kind: 'subscription',
        description: `${p.nameEn} (${PERIOD_DAYS} days)`,
        amountEgp: p.priceMonthlyEgp,
        ref: p.code,
      },
    ]);
    await this.audit.record({
      ...
      targetId: inv.id,
      dealerId,
      metadata: { plan: p.code },
    });
    return {
      ...(await this.checkout(inv, contact, locale)),
      subscription: await this.subscription(dealerId),
    };
  }
  ...
  async feature(
    ...
    if (limits.featuredCreditsRemaining > 0 && days <= 7) {
      await this.db
        .update(subscriptions)
        .set({ featuredCreditsRemaining: sql`${subscriptions.featuredCreditsRemaining} - 1` })
        .where(eq(subscriptions.dealerId, dealerId));
      const placement = await this.placeFeatured(dealerId, listingId, days, null);
      await this.audit.record({
        actorUserId: actor.userId,
        actorRole: actor.role,
        action: 'billing.feature.credit',
        targetType: 'listing',
        targetId: listingId,
        dealerId,
        metadata: { days },
    ...
    const inv = await this.openInvoice(dealerId, [
      {
        ...
        amountEgp: days * this.env.FEATURED_PRICE_PER_DAY_EGP,
        ref: `${listingId}:${days}`,
      },
    ]);
    await this.audit.record({
      actorUserId: actor.userId,
      actorRole: actor.role,
      action: 'billing.feature.checkout',
      targetType: 'invoice',
      targetId: inv.id,
      dealerId,
    });
  ...
  private async placeFeatured(
    ...
    const [current] = await this.db
      ...
      .from(featuredPlacements)
      .where(
        and(eq(featuredPlacements.listingId, listingId), gt(featuredPlacements.endsAt, new Date())),
      )
      .orderBy(desc(featuredPlacements.endsAt))
      .limit(1);
    ...
    const [p] = await this.db
      ...
      .values({ listingId, dealerId, startsAt, endsAt, invoiceId })
      .returning();
    await this.events.publish('featured.activated', {
      listingId,
      dealerId,
  ...
  private async activate(dealerId: string, planCode: string) {
    ...
    await this.db
      ...
      .values({
        ...
        featuredCreditsRemaining: p.featuredCreditsPerMonth,
        gateway: this.gateway.name,
      })
      .onConflictDoUpdate({
        target: subscriptions.dealerId,
        set: {
          planCode,
          status: 'active',
          currentPeriodEnd: periodEnd,
  ...
  async handleWebhook(
    ...
    const inserted = await this.db
      ...
      .values({ gateway: gatewayName, eventId: ev.eventId, payload: ev.raw as object })
      .onConflictDoNothing()
    ...
    if (ev.amountEgp !== inv.totalEgp) {
      this.logger.error(
        { invoice: inv.id, expected: inv.totalEgp, got: ev.amountEgp },
  ...
  async invoice(dealerId: string, id: string) {
    const [inv] = await this.db
      .select()
      .from(invoices)
      .where(and(eq(invoices.id, id), eq(invoices.dealerId, dealerId)));
    if (!inv) throw Errors.notFound('Invoice');
    return inv;
  }
    ...
    const due = await this.db
      ...
      .where(
        and(
          ...
          lt(subscriptions.currentPeriodEnd, soon),
          sql`${subscriptions.planCode} <> ${FREE_PLAN}`,
        ),
      );
    let opened = 0;
    for (const s of due) {
      if (s.cancelAtPeriodEnd) {
        if (s.currentPeriodEnd && s.currentPeriodEnd < now)
          await this.activate(s.dealerId, FREE_PLAN);
        continue;
      }
      const [existing] = await this.db
        ...
        .where(
          and(
            eq(invoices.dealerId, s.dealerId),
            eq(invoices.status, 'open'),
            gt(invoices.createdAt, new Date(now.getTime() - 5 * 86_400_000)),
          ),
        );
      if (!existing) {
        const p = await this.plan(s.planCode);
        await this.openInvoice(s.dealerId, [
          {
            kind: 'subscription',
            description: `${p.nameEn} renewal`,
            amountEgp: p.priceMonthlyEgp,
            ref: p.code,
          },
        ]);
    ...
    const lapsed = await this.db
      ...
      .where(
        and(
          ...
          lt(subscriptions.currentPeriodEnd, new Date(now.getTime() - GRACE_DAYS * 86_400_000)),
        ),
      );
  ...
  async featuredListingIds(): Promise<Set<string>> {
    const rows = await this.db
      .select({ id: featuredPlacements.listingId })
      .from(featuredPlacements)
      .where(
        and(lt(featuredPlacements.startsAt, new Date()), gt(featuredPlacements.endsAt, new Date())),
      );
  ...
  adminInvoices(status?: string) {
    return this.db
      .select()
      .from(invoices)
      .where(status ? eq(invoices.status, status as never) : undefined)
      .orderBy(desc(invoices.createdAt))
      .limit(200);
  }
}
