...
export class MockGateway implements PaymentGateway {
  ...
  async createCheckout(req: CheckoutRequest): Promise<Checkout> {
    return {
      url: `${this.webBase}/en/dealer/billing/mock-checkout?invoice=${req.invoiceId}&amount=${req.totalEgp}`,
      gatewayRef: `mock_${req.invoiceId}`,
    };
  }
...
export class PaymobGateway implements PaymentGateway {
  ...
  async createCheckout(req: CheckoutRequest): Promise<Checkout> {
    ...
    const res = await fetch('https://accept.paymob.com/v1/intention/', {
      ...
      body: JSON.stringify({
        ...
        billing_data: {
          first_name: first || 'Agarha',
          last_name: rest.join(' ') || 'Dealer',
          phone_number: req.customer.phone,
          email: req.customer.email ?? 'billing@agarha.com',
          country: 'EG',
        },
    ...
    return {
      url: `https://accept.paymob.com/unifiedcheckout/?publicKey=${this.publicKey}&clientSecret=${body.client_secret}`,
  ...
  parseWebhook(
    query: Record<string, string>,
    _h: Record<string, string | undefined>,
    rawBody: string,
  ): PaymentEvent | null {
    ...
    return {
      eventId: String(body.obj.id),
      invoiceId,
      gatewayRef: String(body.obj.order?.id ?? ''),
      success: body.obj.success === true,
      amountEgp: Math.round(body.obj.amount_cents / 100),
      raw: body,
    };
  }
}
