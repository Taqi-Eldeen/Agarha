import { SetMetadata, UseInterceptors, applyDecorators, type CallHandler, type ExecutionContext, Injectable, type NestInterceptor } from '@nestjs/common';
...
export class CacheControlInterceptor implements NestInterceptor {
  ...
  intercept(ctx: ExecutionContext, next: CallHandler): Observable<unknown> {
    const seconds = this.reflector.getAllAndOverride<number | undefined>(PUBLIC_CACHE, [ctx.getHandler(), ctx.getClass()]);
    const res = ctx.switchToHttp().getResponse<Response>();
    res.setHeader(
      'Cache-Control',
      seconds ? `public, max-age=0, s-maxage=${seconds}, stale-while-revalidate=${seconds * 5}` : 'no-store',
    );
