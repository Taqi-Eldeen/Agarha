...
export class AuthGuard implements CanActivate {
  ...
  async canActivate(ctx: ExecutionContext): Promise<boolean> {
    const meta = this.reflector.getAllAndOverride<AuthMeta | undefined>(AUTH_META, [
      ctx.getHandler(),
      ctx.getClass(),
    ]);
    if (!meta) return true;
...
export const Auth = (scope: SessionScope, ...roles: Role[]) =>
  applyDecorators(
    SetMetadata(AUTH_META, { scope, roles } satisfies AuthMeta),
    UseGuards(AuthGuard),
    ApiBearerAuth(),
    ApiCookieAuth(),
  );
...
export const OptionalAuth = (scope: SessionScope) =>
  applyDecorators(
    SetMetadata(AUTH_META, { scope, optional: true } satisfies AuthMeta),
    UseGuards(AuthGuard),
  );
...
export const CurrentDealer = createParamDecorator(
  (_: unknown, ctx: ExecutionContext): { dealerId: string; userId: string; role: Role } => {
    const auth = ctx.switchToHttp().getRequest<Request>().auth;
...
export const AdminAuth = (...roles: Role[]) =>
  applyDecorators(
    ...
    Auth('admin', ...(roles.length ? roles : (['admin'] as Role[]))),
  );
