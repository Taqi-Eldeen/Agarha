...
describe('P3 public web API', () => {
  ...
  const ids = (r: { body: { items: { card: { id: string } }[] } }) =>
    r.body.items.map((i) => i.card.id);
  ...
  it('filters by type, transmission, seats, driver option, airport and price for the chosen period', async () => {
    ...
    expect(new Set(ids(await t.http.get('/v1/search?driver=self')))).toEqual(
      new Set([cheap, manual]),
    );
    expect(new Set(ids(await t.http.get('/v1/search?driver=driver')))).toEqual(
      new Set([manual, suv]),
    );
  ...
  it('sorts by price and paginates with cursors', async () => {
    ...
    const next = await t.http
      .get(`/v1/search?sort=price_asc&limit=2&cursor=${asc.body.nextCursor}`)
      .expect(200);
  ...
  it('matches Arabic queries with alef/taa-marbuta variants and typos', async () => {
    ...
    expect(
      new Set(ids(await t.http.get(`/v1/search?q=${encodeURIComponent('مدينه نصر')}`))),
    ).toEqual(new Set([cheap, manual, suv]));
    expect(ids(await t.http.get('/v1/search?q=sportge'))).toContain(suv);
    const sug = await t.http
      .get(`/v1/catalog/suggest?q=${encodeURIComponent('كورولا')}`)
      .expect(200);
    expect(sug.body.models[0].nameEn).toBe('Corolla');
  });

  it('near me, bounding box and map pins', async () => {
    const near = await t.http
      .get('/v1/search?lat=30.05&lng=31.33&radiusKm=20&sort=distance')
      .expect(200);
    ...
    expect(pins.body.items[0]).toEqual(
  ...
  it('listing page shows facts, freshness, the deposit notice and a dealer card with trust signals', async () => {
    ...
    expect(r.body.listing).toMatchObject({
      freshness: 'fresh',
      minAge: 23,
      kmLimitPerDay: 200,
      prices: { day: 900, deposit: 3000 },
    });
    expect(r.body.safety).toEqual({ neverPayDepositBeforeSeeing: true, agarhaIsNotAParty: true });
    expect(r.body.dealer).toMatchObject({
      verified: true,
      reviews: { count: 0 },
      responseRate: null,
    });
  ...
  it('logs a lead anonymously and returns a wa.me link with the reference code (idempotent)', async () => {
    ...
    const r = await t.http
      ...
      .send({ listingId: cheap, channel: 'whatsapp', locale: 'ar' })
      .expect(201);
    expect(r.body.refCode).toMatch(/^AG-[2-9A-HJ-NP-Z]{4}$/);
    expect(r.body.url).toMatch(/^https:\/\/wa\.me\/201020000001\?text=/);
    expect(decodeURIComponent(r.body.url.split('text=')[1])).toBe(
      `مرحباً، أستفسر عن تويوتا كورولا ٢٠٢٤ على أجّرها (Ref ${r.body.refCode})`,
    );
    ...
    const call = await t.http
      .post('/v1/leads')
      .set('x-forwarded-for', ip)
      .send({ listingId: cheap, channel: 'call' })
      .expect(201);
    ...
    expect(dealerLeads.body.items[0]).toMatchObject({
      refCode: call.body.refCode,
      channel: 'call',
      listing: { nameAr: 'تويوتا كورولا' },
    });
    await t.http
      .put(`/v1/dealer/leads/${dealerLeads.body.items[0].id}/outcome`)
      .set(bearer(d.token))
      .send({ outcome: 'from_agarha' })
      .expect(200);
    // Dealer WhatsApp alert is queued (sent by the worker), with the ref code.
    const [delivery] = await t.db
      .select()
      .from(notificationDeliveries)
      .where(eq(notificationDeliveries.template, 'lead_alert'));
    expect(delivery).toMatchObject({
      channel: 'whatsapp',
      recipient: '+201020000001',
      status: 'queued',
    });
    await t.app.get(NotificationsService, { strict: false }).deliver(delivery!.id, 1);
    expect(Outbox.latestTo('+201020000001', 'whatsapp')!.body).toContain(
      r.body.refCode.slice(0, 3),
    );
  });
  ...
  it('rate-limits leads per IP and listing', async () => {
    ...
    for (let i = 0; i < 5; i++)
      await t.http
        .post('/v1/leads')
        .set('x-forwarded-for', ip)
        .send({ listingId: suv, channel: 'call' })
        .expect(201);
    await t.http
      .post('/v1/leads')
      .set('x-forwarded-for', ip)
  ...
  it('reports, favorites and saved searches need a phone-OTP account', async () => {
    await t.http
      .post('/v1/reports')
      .send({ listingId: cheap, reason: 'scam_or_deposit_request' })
      .expect(401);
    ...
    const rep = await t.http
      ...
      .send({
        listingId: cheap,
        reason: 'scam_or_deposit_request',
        details: 'asked for deposit by bank transfer',
      })
      .expect(201);
    ...
    const saved = await t.http
      ...
      .set(bearer(c.token))
      .send({ name: 'SUVs', query: { city: 'cairo', type: 'suv' } })
      .expect(201);
    await t.http
      .patch(`/v1/me/saved-searches/${saved.body.id}`)
      .set(bearer(c.token))
      .send({ alertsEnabled: true })
      .expect(200);
    await t.db.execute(
      `UPDATE saved_searches SET last_notified_at = now() - interval '1 day'` as never,
    );
  ...
  it('dealer directory, profile, landing pages and sitemap data', async () => {
    ...
    expect(dir.body.items[0]).toMatchObject({
      nameEn: 'Nile Rentals',
      verified: true,
      branchCount: 1,
    });
  ...
  it('account deletion unlinks leads but keeps them for the dealer', async () => {
    ...
    const lead = await t.http
      ...
      .send({ listingId: manual, channel: 'whatsapp' })
      .expect(201);
