...
export async function jpeg(w = 1200, h = 800): Promise<Buffer> {
  return sharp({ create: { width: w, height: h, channels: 3, background: { r: 200, g: 40, b: 40 } } }).jpeg().withMetadata({ exif: { IFD0: { Make: 'Phone' } } }).toBuffer();
}
...
export async function drainMedia(t: TestApp) {
  ...
  for (const j of jobs) {
    ...
    if (d.kind === 'listing_photo') await t.app.get(ListingsService, { strict: false }).processPhoto(d.photoId!);
...
export async function verifiedDealer(t: TestApp, phone: string) {
  ...
  const biz = await t.http.post('/v1/dealer/onboarding/business').set(bearer(owner.token)).send({ legalName: 'Nile Rentals LLC', displayNameAr: 'النيل لتأجير السيارات', displayNameEn: 'Nile Rentals', commercialRegistrationNo: '123456', taxCardNo: '123-456-789', phone: '0223456789', whatsapp: phone, client: 'mobile' }).expect(201);
  const token = biz.body.tokens.accessToken as string;
  await t.http.post('/v1/dealer/branches').set(bearer(token)).send({ areaId: ids.areaId, nameAr: 'فرع مدينة نصر', nameEn: 'Nasr City branch', addressEn: 'Abbas El Akkad St' }).expect(201);
  const pdf = Buffer.from('%PDF-1.4\n% test document\n');
  for (const type of ['commercial_registration', 'tax_card', 'owner_national_id']) {
    const up = await t.http.post('/v1/dealer/documents/uploads').set(bearer(token)).send({ type, mimeType: 'application/pdf', sizeBytes: pdf.length, sha256: 'a'.repeat(64) }).expect(201);
    await upload(t, up.body.upload.url, pdf, 'application/pdf').expect(200);
    await t.http.post(`/v1/dealer/documents/${up.body.documentId}/complete`).set(bearer(token)).expect(200);
  }
  await drainMedia(t);
  ...
  const docs = await t.http.get(`/v1/admin/dealers/${biz.body.dealer.id}/documents`).set(bearer(admin.token)).expect(200);
  for (const d of docs.body.items) await t.http.post(`/v1/admin/documents/${d.id}/review`).set(bearer(admin.token)).send({ approve: true }).expect(200);
  await t.http.post(`/v1/admin/dealers/${biz.body.dealer.id}/verify`).set(bearer(admin.token)).expect(200);
  return { ...ids, token, dealerId: biz.body.dealer.id as string, admin, totpSecret: owner.totpSecret, currentTotp };
}

/** Creates a car with one processed photo and publishes it; approves it if pre-moderated. */
export async function liveListing(t: TestApp, d: Awaited<ReturnType<typeof verifiedDealer>>, overrides: Record<string, unknown> = {}) {
  const branches = await t.http.get('/v1/dealer/branches').set(bearer(d.token)).expect(200);
  const car = await t.http.post('/v1/dealer/listings').set(bearer(d.token)).send({ ...LISTING, carModelId: d.modelId, branchId: branches.body.items[0].id, ...overrides });
  if (car.status !== 201) throw new Error(`create listing ${car.status}: ${JSON.stringify(car.body)}`);
  const img = await jpeg();
  const ph = await t.http.post(`/v1/dealer/listings/${car.body.id}/photos`).set(bearer(d.token)).send({ mimeType: 'image/jpeg', sizeBytes: img.length }).expect(201);
  ...
  await drainMedia(t);
  const pub = await t.http.post(`/v1/dealer/listings/${car.body.id}/publish`).set(bearer(d.token)).expect(200);
  if (pub.body.status === 'pending') await t.http.post(`/v1/admin/listings/${car.body.id}/moderate`).set(bearer(d.admin.token)).send({ decision: 'approve' }).expect(200);
