...
export async function resetData(t: TestApp): Promise<void> {
  const r = await t.db.execute<{ tablename: string }>(
    sql`SELECT tablename FROM pg_tables WHERE schemaname = 'public' AND tablename NOT IN ('spatial_ref_sys')`,
  );
...
export const freshIp = () =>
  `10.${(process.pid % 200) + 1}.${Math.floor(ipCounter / 250)}.${(ipCounter++ % 250) + 1}`;
...
export async function dealerOwner(t: TestApp, phone: string, ip = freshIp()) {
  const r = await t.http
    .post('/v1/auth/otp/request')
    .set('x-forwarded-for', ip)
    .send({ phone, purpose: 'dealer_sign_up', turnstileToken: 'pass' })
    .expect(200);
  const proof = await t.http
    .post('/v1/auth/otp/proof')
    .set('x-forwarded-for', ip)
    .send({ challengeId: r.body.challengeId, phone, code: lastCode(normalize(phone)) })
    .expect(200);
  ...
  const conf = await t.http
    .post('/v1/auth/dealer/totp/confirm')
    .send({
      setupToken: reg.body.setupToken,
      code: currentTotp(reg.body.totp.secret),
      client: 'mobile',
    })
    .expect(200);
  return {
    token: conf.body.tokens.accessToken as string,
    totpSecret: reg.body.totp.secret as string,
  };
}

export async function adminSession(
  t: TestApp,
  email = 'ops@agarha.com',
  roles: string[] = ['admin'],
) {
  const users = t.app.get(UsersService, { strict: false });
  const u = await users.findOrCreateByPhone(
    `+2011${String(Math.floor(Math.random() * 1e8)).padStart(8, '0')}`,
    'en',
  );
  ...
  const setup = await t.http
    .post('/v1/auth/admin/totp/setup')
    .send({ setupToken: step1.body.setupToken, code: currentTotp(step1.body.totp.secret) })
    .expect(200);
  const cookies = setup.headers['set-cookie'] as unknown as string[];
  const at = cookies
    .find((c) => c.startsWith('ag_a_at='))!
    .split(';')[0]!
    .split('=')[1]!;
