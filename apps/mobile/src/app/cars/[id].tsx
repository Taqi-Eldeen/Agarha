...
export default function ListingScreen() {
  ...
  if (q.isError || !q.data)
    return (
      <View className="flex-1 bg-page p-4">
        {(q.error as { status?: number } | null)?.status === 404 ? <EmptyState title={t('notFoundTitle')} body={t('notFoundBody')} /> : <ErrorState body={ts('errorBody')} onRetry={() => void q.refetch()} />}
      </View>
    );
  ...
  const photos = listing.photos
    ...
    .map((p) => ({ id: p.id, src: p.urls!.webp!['1280'] ?? p.urls!.webp!['640'] ?? '', blurhash: p.blurhash }));
  ...
  const facts: [string, string][] = [
    ...
    [t('driverOption'), listing.driverOption === 'self' ? tu('selfDrive') : listing.driverOption === 'driver' ? tu('withDriver') : tu('selfOrDriver')],
    ...(listing.deliveryOptions.length ? [[t('delivery'), listing.deliveryOptions.map((d) => t(`deliveryOptions.${d}` as never)).join('، ')] as [string, string]] : []),
  ];
  const share = async () => {
  ...
  return (
    <View className="flex-1 bg-page">
        ...
        options={{
          ...
          headerRight: () => (
            <View className="flex-row gap-1">
              <IconButton label={t('share')} icon={<Share2 size={22} color={colors.textPrimary} strokeWidth={1.75} />} onPress={() => void share()} />
      ...
      <ScrollView contentContainerClassName="pb-6">
        ...
        <View className="gap-6 p-4">
          <View className="gap-2">
            <Text variant="h1" accessibilityRole="header">{name}</Text>
            ...
            <Text variant="caption" tone="secondary">{t('lastConfirmed', { ago: ago(card.lastConfirmedAt) })}</Text>
          </View>
          {!card.available ? <InlineAlert tone="warning">{t('unavailableNotice')}</InlineAlert> : null}
          <ChipGroup label={ts('period')} single value={[period]} onChange={(v) => setPeriod((v[0] as PricePeriod) ?? 'day')} options={[{ value: 'day', label: ts('periodDay') }, { value: 'week', label: ts('periodWeek') }, { value: 'month', label: ts('periodMonth') }]} />

          <View className="gap-3">
            <Text variant="h2">{t('requirements')}</Text>
            <RequirementList deposit={card.prices.deposit} minAge={card.minAge} requiredDocs={card.requiredDocs} kmLimitPerDay={card.kmLimitPerDay} airportPickup={card.airportPickup} />
          </View>
          ...
          <View className="gap-2">
            ...
            <DealerCard
              ...
              responseRateLabel={dealer.responseRate === null ? '' : t('responseRate', { percent: Math.round(dealer.responseRate * 100) })}
              memberSinceLabel={t('memberSince', { date: format.dateTime(new Date(dealer.memberSince), { month: 'long', year: 'numeric' }) })}
            />
          </View>
      ...
      <ContactBar prices={card.prices} period={period} onContact={(c) => void contact(c)} contacting={busy} />
    </View>
  );
}
