...
export default function Report() {
  ...
  if (!signedIn)
    return (
      <Screen edges={['bottom']}>
        <EmptyState
          body={t('errors.unauthorized')}
          action={<Button onPress={() => router.replace('/sign-in')}>{t('common.signIn')}</Button>}
        />
      </Screen>
    );
  ...
  const submit = async () => {
    ...
    try {
      await api.POST('/v1/reports', {
        body: { listingId: id, reason, ...(details.trim() ? { details: details.trim() } : {}) },
      });
  ...
  return (
    <Screen edges={['bottom']}>
      ...
      <ChipGroup
        label={t('web.report.reason')}
        single
        value={reason ? [reason] : []}
        onChange={(v) => setReason((v[0] as (typeof REASONS)[number]) ?? null)}
        options={REASONS.map((r) => ({ value: r, label: t(`web.report.reasons.${r}`) }))}
      />
      <TextField
        label={t('web.report.details')}
        value={details}
