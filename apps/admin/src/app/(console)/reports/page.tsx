...
type Report = {
  ...
  dealerId: string;
  reason: keyof (typeof messages)['en']['web']['report']['reasons'];
  details: string | null;
  createdAt: string;
};
      ...
      <ul className="flex flex-col gap-3">
        {q.data?.items.map((r) => (
          ...
          >
            ...
            <p dir="ltr" className="text-caption text-fg-secondary">
              {r.listingId} · {new Date(r.createdAt).toISOString()}
            </p>
            <TextField
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <Button
                size="sm"
                variant="danger"
                disabled={(note[r.id] ?? '').length < 3 || !r.listingId}
                onClick={() => void resolve(r.id, 'hide_listing')}
              >
                {t('reports.hideListing')}
              </Button>
              <Button
                size="sm"
                variant="danger"
                disabled={(note[r.id] ?? '').length < 3}
                onClick={() => void resolve(r.id, 'suspend_dealer')}
              >
                {t('reports.suspendDealer')}
              </Button>
            </div>
          </li>
        ))}
