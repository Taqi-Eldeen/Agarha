...
export default function Dealers() {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <ChipGroup
        label={t('dealers.status')}
        single
        value={[status]}
        onChange={(v) => setStatus(v[0] ?? 'all')}
        options={[
          { value: 'pending_review', label: t('dealers.queue') },
          { value: 'all', label: t('dealers.all') },
          { value: 'verified', label: t('dealers.statuses.verified') },
          { value: 'onboarding', label: t('dealers.statuses.onboarding') },
      ...
      <DataTable
        ...
        columns={[
          {
            key: 'name',
            header: t('catalog.nameEn'),
            cell: (d) => (
              <Link href={`/dealers/${d.id}`} className="font-semibold text-brand hover:underline">
                {locale === 'ar' ? d.displayNameAr : d.displayNameEn}
              </Link>
            ),
          },
          ...
          {
            key: 'status',
            header: t('dealers.status'),
            cell: (d) =>
              d.suspendedAt ? (
                <Badge kind="stale">{t('dealers.suspendedBadge')}</Badge>
              ) : (
                t(`dealers.statuses.${d.status as 'verified'}`)
              ),
          },
          {
            key: 'updated',
            header: t('audit.time'),
            cell: (d) =>
              new Date(d.updatedAt).toLocaleString(locale === 'ar' ? 'ar-EG-u-nu-latn' : 'en-GB'),
