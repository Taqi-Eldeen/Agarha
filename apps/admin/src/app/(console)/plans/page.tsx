        ...
        {plans.data?.items.map((p) => (
          ...
          >
            ...
            <TextField
              ...
              inputMode="numeric"
              dir="ltr"
              defaultValue={String(p.priceMonthlyEgp)}
              onChange={(e) => setEdit({ ...edit, [p.code]: e.target.value })}
            />
            <p className="text-caption text-fg-secondary">
              {t('plans.live')}: {p.maxLiveListings ?? '∞'} · {t('plans.team')}: {p.maxTeamMembers}{' '}
              · {t('plans.credits')}: {p.featuredCreditsPerMonth}
            </p>
            <Button
              size="sm"
              ...
              onClick={async () => {
                try {
                  await adminFetch(`/admin/plans/${p.code}`, {
                    method: 'PUT',
                    json: { ...p, priceMonthlyEgp: Number(edit[p.code] ?? p.priceMonthlyEgp) },
                  });
      ...
      <DataTable
        ...
        columns={[
          ...
          { key: 's', header: t('dealers.status'), cell: (i) => i.status },
          {
            ...
            cell: (i) => formatEgp(i.totalEgp, locale),
            numeric: true,
            sortValue: (i) => i.totalEgp,
          },
          {
            key: 'd',
            header: t('audit.time'),
            cell: (i) => i.createdAt.slice(0, 10),
