    ...
    <div className="flex flex-col gap-8">
      <JsonLdScript
        data={{
          ...
          '@type': 'ItemList',
          name: title,
          itemListElement: data.listings.map((c, i) => ({ '@type': 'ListItem', position: i + 1, url: `${siteUrl}/${locale}${listingPath(c)}` })),
        }}
      />
      <header className="flex flex-col gap-2">
        <h1 className="text-h1">{title}</h1>
        <p className="text-fg-secondary">{landingIntro(locale, data)}</p>
        {data.stats.median ? <p className="text-caption text-fg-secondary">{t('median', { price: formatEgp(data.stats.median, locale) })}</p> : null}
      </header>
      {data.listings.length ? (
        <section>
          ...
          <Link href={searchHref} className="mt-4 inline-flex min-h-touch items-center font-medium text-brand hover:underline">
      ...
      <div className="grid gap-8 md:grid-cols-2">
        <section>
          ...
          <ul className="flex flex-wrap gap-2">
            {data.areas.map((a) => (
              <li key={a.slug}>
                <Link href={`/${data.city.slug}/${a.slug}`} className="inline-flex min-h-touch items-center gap-2 rounded-full border border-border bg-card px-4 hover:bg-brand-subtle">
        ...
        <section>
          ...
          <ul className="flex flex-wrap gap-2">
            {data.types.map((ty) => (
              <li key={ty.type}>
                <Link href={`/${data.city.slug}/${ty.type}`} className="inline-flex min-h-touch items-center gap-2 rounded-full border border-border bg-card px-4 hover:bg-brand-subtle">
