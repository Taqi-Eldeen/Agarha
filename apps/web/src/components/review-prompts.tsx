...
export function ReviewPrompts() {
  ...
  const q = useQuery({
    queryKey: ['reviewable'],
    queryFn: async () =>
      (await api.GET('/v1/me/reviewable')).data as unknown as { items: Reviewable[] },
  });
      ...
      {q.data.items.slice(0, 5).map((r) => {
        ...
        return (
          <form
            ...
            onSubmit={async (e) => {
              ...
              await api.POST('/v1/reviews', {
                body: { leadId: r.leadId, rating: s.rating, ...(s.body ? { body: s.body } : {}) },
              });
          ...
          >
            <RatingStars
              value={s.rating}
              onChange={(rating) => setState({ ...state, [r.leadId]: { ...s, rating } })}
            />
            <TextField
              label={t('body')}
              value={s.body}
              onChange={(e) => setState({ ...state, [r.leadId]: { ...s, body: e.target.value } })}
              maxLength={1000}
            />
