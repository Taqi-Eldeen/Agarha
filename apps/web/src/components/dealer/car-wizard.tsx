...
export function CarWizard({
  ...
  listing?: Listing & { model?: { makeId: string } | null };
}) {
  ...
  const form = useForm<Form>({
    ...
    resolver: schemaResolver(listingFactsSchema, toPayload, known),
  });
  const f = form.watch();

  const makes = useQuery({
    queryKey: ['makes'],
    queryFn: async () => (await api.GET('/v1/catalog/makes')).data!.items,
    staleTime: 3_600_000,
  });
  const models = useQuery({
    queryKey: ['models', f.makeId],
    enabled: !!f.makeId,
    queryFn: async () =>
      (await api.GET('/v1/catalog/makes/{id}/models', { params: { path: { id: f.makeId! } } }))
        .data!.items,
  });
  const branches = useQuery({
    queryKey: ['branches'],
    queryFn: async () =>
      (await api.GET('/v1/dealer/branches')).data as unknown as {
        items: { id: string; nameAr: string; nameEn: string; isPrimary: boolean }[];
      },
  });
  useEffect(() => {
  ...
  const set = <K extends keyof Form>(k: K, v: Form[K]) =>
    form.setValue(k, v as never, { shouldDirty: true, shouldValidate: form.formState.isSubmitted });
  ...
  const save = async () => {
    ...
    try {
      if (id)
        await api.PUT('/v1/dealer/listings/{id}', {
          params: { path: { id } },
          body: payload() as never,
        });
      else {
        const { data } = await api.POST('/v1/dealer/listings', {
          body: payload() as never,
  ...
  const addFiles = async (files: File[]) => {
    ...
    for (const file of files) {
      ...
      setUploads((u) => [
        ...u,
        { id: tmp, previewUrl: URL.createObjectURL(file), status: 'uploading', progress: 0, file },
      ]);
      try {
        const photoId = await uploadPhoto(api, id, file, (p) =>
          setUploads((u) => u.map((x) => (x.id === tmp ? { ...x, progress: p } : x))),
        );
        setUploads((u) =>
          u.map((x) => (x.id === tmp ? { ...x, id: photoId, status: 'processing' } : x)),
        );
      } catch {
  ...
  useEffect(() => {
    ...
    const i = setInterval(async () => {
      const { data } = await api.GET('/v1/dealer/listings/{id}/photos', {
        params: { path: { id } },
      });
      const items = (
        data as unknown as {
          items: { id: string; status: string; urls: { webp: Record<string, string> } | null }[];
        }
      ).items;
      setUploads((u) =>
        u.map((x) => {
          const s = items.find((p) => p.id === x.id);
          return s
            ? {
                ...x,
                status:
                  s.status === 'ready' ? 'ready' : s.status === 'rejected' ? 'failed' : x.status,
                previewUrl: s.urls?.webp['320'] ?? x.previewUrl,
              }
            : x;
        }),
      );
    ...
    <div className="mx-auto max-w-2xl">
      ...
      <Wizard
        ...
        onNext={async () => {
          ...
          if (!id) return;
          try {
            const { data } = await api.POST('/v1/dealer/listings/{id}/publish', {
              params: { path: { id } },
            });
            toast({
              tone: 'success',
              text:
                (data as unknown as { status: string }).status === 'pending'
                  ? t('submitted')
                  : t('published'),
            });
        ...
        {step === 0 ? (
          <div className="flex flex-col gap-4">
            <Combobox
              ...
              onValueChange={(v) => {
                set('makeId', v);
                set('carModelId', undefined);
              }}
              options={(makes.data ?? []).map((m) => ({
                value: m.id,
                label: locale === 'ar' ? m.nameAr : m.nameEn,
                hint: locale === 'ar' ? m.nameEn : m.nameAr,
              }))}
            />
            <Combobox
              label={t('model')}
              value={f.carModelId}
              onValueChange={(v) => set('carModelId', v)}
              error={e('carModelId')}
              disabled={!f.makeId}
              options={(models.data ?? []).map((m) => ({
                value: m.id,
                label: locale === 'ar' ? m.nameAr : m.nameEn,
                hint: tu(`bodyTypes.${m.bodyType}`),
              }))}
            />
            ...
            <ChipGroup
              ...
              onChange={(v) => v[0] && set('transmission', v[0] as Form['transmission'])}
              options={[
            ...
            <Select
              label={t('fuel')}
              value={f.fuel}
              onValueChange={(v) => set('fuel', v as Form['fuel'])}
              options={FUELS.map((x) => ({ value: x, label: tl(`fuels.${x}`) }))}
            />
            <TextField
              label={t('seats')}
              inputMode="numeric"
              value={f.seats}
              onChange={(ev) => set('seats', ev.target.value)}
              error={e('seats')}
              dir="ltr"
            />
            <Select
            ...
            />
            <ChipGroup
              label={t('driverOption')}
              single
              value={[f.driverOption]}
              onChange={(v) => v[0] && set('driverOption', v[0] as Form['driverOption'])}
              options={[
                { value: 'self', label: tu('selfDrive') },
                { value: 'driver', label: tu('withDriver') },
                { value: 'both', label: tu('selfOrDriver') },
              ]}
            />
          </div>
        ) : null}
        {step === 1 ? (
          <div className="flex flex-col gap-4">
            <TextField
              ...
              inputMode="numeric"
              value={f.priceDayEgp}
              onChange={(ev) => set('priceDayEgp', ev.target.value)}
              error={e('priceDayEgp')}
              dir="ltr"
            />
            <TextField
              label={t('priceWeek')}
              inputMode="numeric"
              value={f.priceWeekEgp}
              onChange={(ev) => set('priceWeekEgp', ev.target.value)}
              error={e('priceWeekEgp')}
              dir="ltr"
            />
            <TextField
              label={t('priceMonth')}
              inputMode="numeric"
              value={f.priceMonthEgp}
              onChange={(ev) => set('priceMonthEgp', ev.target.value)}
              error={e('priceMonthEgp')}
              dir="ltr"
            />
            <TextField
              label={t('deposit')}
              ...
              inputMode="numeric"
              value={f.depositEgp}
              onChange={(ev) => set('depositEgp', ev.target.value)}
              error={e('depositEgp')}
              dir="ltr"
            />
          </div>
        ...
        {step === 2 ? (
          <div className="flex flex-col gap-4">
            <TextField
              ...
              inputMode="numeric"
              value={f.minAge}
              onChange={(ev) => set('minAge', ev.target.value)}
              error={e('minAge')}
              dir="ltr"
            />
            <div className="flex flex-col gap-2">
              ...
              <ChipGroup
                label={t('docs')}
                value={f.requiredDocs}
                onChange={(v) => set('requiredDocs', v)}
                options={REQUIRED_DOCS.map((d) => ({ value: d, label: tu(`docs.${d}`) }))}
              />
              {e('requiredDocs') ? (
            ...
            <TextField
              ...
              hint={t('kmUnlimited')}
              inputMode="numeric"
              value={f.kmLimitPerDay}
              onChange={(ev) => set('kmLimitPerDay', ev.target.value)}
              error={e('kmLimitPerDay')}
              dir="ltr"
            />
            <div className="flex flex-col gap-2">
            ...
            <label className="flex min-h-touch items-center gap-3">
              <input
                type="checkbox"
                className="size-5"
                checked={f.airportPickup}
                onChange={(ev) => set('airportPickup', ev.target.checked)}
              />
              {t('airport')}
            </label>
            <TextField
              ...
              onChange={(ev) => set('descriptionAr', ev.target.value)}
              optional
            />
          </div>
        ) : null}
        {step === 3 ? (
          <div className="flex flex-col gap-3">
            ...
            <PhotoUploader
              ...
              onRemove={async (uid) => {
                setUploads((u) => u.filter((x) => x.id !== uid));
                if (id && !uid.startsWith('local-'))
                  await api.DELETE('/v1/dealer/listings/{id}/photos/{photoId}', {
                    params: { path: { id, photoId: uid } },
                  });
              }}
              onReorder={async (ids) => {
                ...
                if (id && ready.length)
                  await api.PUT('/v1/dealer/listings/{id}/photos/order', {
                    params: { path: { id } },
                    body: { photoIds: ready },
                  });
              }}
