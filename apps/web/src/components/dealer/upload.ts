...
export async function uploadDocument(api: ApiClient, type: string, file: File) {
  ...
  const { data } = await api.POST('/v1/dealer/documents/uploads', {
    body: { type: type as never, mimeType, sizeBytes: body.size, sha256: await sha256Hex(body) },
  });
...
export async function uploadPhoto(
  ...
  const { data } = await api.POST('/v1/dealer/listings/{id}/photos', {
    params: { path: { id: listingId } },
    body: { mimeType: 'image/jpeg', sizeBytes: body.size },
  });
  ...
  await api.POST('/v1/dealer/listings/{id}/photos/{photoId}/complete', {
    params: { path: { id: listingId, photoId: r.photoId } },
  });
  return r.photoId;
}
