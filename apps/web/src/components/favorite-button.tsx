...
export function FavoriteButton({ card, variant = 'overlay' }: { card: ListingCard; variant?: 'overlay' | 'inline' }) {
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={variant === 'overlay' ? 'inline-flex size-11 items-center justify-center rounded-full bg-card/90 shadow-1' : 'inline-flex min-h-touch items-center gap-2 rounded-md border border-border px-3'}
    >
      <Heart aria-hidden className={on ? 'size-5 fill-danger text-danger' : 'size-5'} strokeWidth={1.75} />
