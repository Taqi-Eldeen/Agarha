...
export function AvailabilityDrawer({ listingId, open, setOpen }: { listingId: string; open: boolean; setOpen: (o: boolean) => void }) {
  ...
  const toInput = (v: AvailabilityForm) => ({ listingId, startDate: v.startDate, endDate: v.endDate, locale, ...(v.note.trim() ? { note: v.note.trim() } : {}) });
  const form = useForm<AvailabilityForm>({ defaultValues: { startDate: today, endDate: today, note: '' }, resolver: schemaResolver(availabilityRequestInputSchema, toInput, known) });
  const submit = form.handleSubmit(async (v) => {
    try {
      ...
      if (!applyServerErrors(form, e)) toast({ tone: 'danger', text: text(e instanceof ApiRequestError ? e.code : 'internal_error') ?? '' });
    }
  });
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-4">
        ...
        <TextField label={t('availabilityFrom')} type="date" min={today} {...form.register('startDate')} error={err('startDate')} dir="ltr" />
        <TextField label={t('availabilityTo')} type="date" min={form.watch('startDate')} {...form.register('endDate')} error={err('endDate')} dir="ltr" />
        <TextField label={t('availabilityNote')} {...form.register('note')} error={err('note')} maxLength={300} />
      </div>
    </Drawer>
  );
}
