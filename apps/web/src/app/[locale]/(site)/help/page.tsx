...
export default async function HelpPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return (
    <div className="mx-auto flex max-w-3xl flex-col gap-8">
      <JsonLdScript data={{ '@context': 'https://schema.org', '@type': 'FAQPage', mainEntity: QS.map((i) => ({ '@type': 'Question', name: t(`faq.q${i}`), acceptedAnswer: { '@type': 'Answer', text: t(`faq.a${i}`) } })) }} />
      <h1 className="text-h1">{t('title')}</h1>
      <section aria-labelledby="safety" className="rounded-lg border border-stale/40 bg-stale/10 p-4">
        ...
        <ul className="list-disc ps-6">
          {(['s1', 's2', 's3', 's4'] as const).map((k) => (
            <li key={k} className="py-1">{t(`safety.${k}`)}</li>
          ))}
        </ul>
      </section>
      <section aria-labelledby="faq">
        <h2 id="faq" className="mb-3 text-h2">{t('faqTitle')}</h2>
        {QS.map((i) => (
          <details key={i} className="border-b border-border py-3">
            <summary className="min-h-touch cursor-pointer py-2 font-semibold">{t(`faq.q${i}`)}</summary>
