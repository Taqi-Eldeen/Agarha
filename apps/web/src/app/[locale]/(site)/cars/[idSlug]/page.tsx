...
export async function generateMetadata({ params }: P): Promise<Metadata> {
  ...
  const vars = {
    ...
    year: d.card.year,
    area: d.card.area[l],
    price: formatEgp(d.card.prices.day, l),
    deposit: formatEgp(d.card.prices.deposit, l),
    dealer: l === 'ar' ? d.dealer.nameAr : d.dealer.nameEn,
  };
  const img = d.card.photo?.url640;
  return {
    title: t('listingTitle', vars),
    description: t('listingDescription', vars),
    alternates: alternates(locale, listingPath(d.card)),
...
export default async function ListingPage({ params }: P) {
  ...
  const d = await serverApi.listing(parseListingParam(idSlug));
  if (!d) notFound();
  if (idSlug !== `${d.card.id}-${d.card.slug}`)
    permanentRedirect(`/${locale}${listingPath(d.card)}`);
  ...
  const photos = L.photos
    ...
    .map((p) => ({
      id: p.id,
      src: p.urls!.webp!['1280']!,
      srcSet: `${p.urls!.webp!['640']} 640w, ${p.urls!.webp!['1280']} 1280w`,
      blurhash: p.blurhash,
    }));
  const facts: [string, string][] = [
    ...
    [
      t('delivery'),
      L.deliveryOptions
        .map((o) => t(`deliveryOptions.${o}` as 'deliveryOptions.branch_pickup'))
        .join('، '),
    ],
  ];
  const notice = (
  ...
  return (
    <article className="flex flex-col gap-6 pb-40 lg:pb-0">
      ...
      <nav aria-label={tn('breadcrumb')} className="text-caption text-fg-secondary">
        <Link href={`/${d.card.city.slug}`} className="hover:underline">
          {d.card.city[l]}
        </Link>
        <span aria-hidden> / </span>
        <Link href={`/${d.card.city.slug}/${d.card.area.slug}`} className="hover:underline">
          {d.card.area[l]}
        </Link>
      </nav>
        ...
        <div className="flex flex-col gap-6">
          ...
          <header className="flex flex-col gap-3">
            <div className="flex flex-wrap items-center gap-2">
              ...
              {L.freshness === 'fresh' ? (
                <Badge kind="fresh">
                  {tu('confirmedAgo', { ago: formatRelative(new Date(L.lastConfirmedAt), l) })}
                </Badge>
              ) : L.freshness === 'aging' ? (
                <Badge kind="stale">
                  {tu('confirmedAgo', { ago: formatRelative(new Date(L.lastConfirmedAt), l) })}
            ...
            <p className="text-caption text-fg-secondary">
              {t('lastConfirmed', { ago: formatRelative(new Date(L.lastConfirmedAt), l) })}
            </p>
          ...
          <section aria-labelledby="req">
            <h2 id="req" className="mb-3 text-h2">
              {t('requirements')}
            </h2>
            <RequirementList
          ...
          <section aria-labelledby="facts">
            <h2 id="facts" className="mb-3 text-h2">
              {t('facts')}
            </h2>
          ...
          {((l === 'ar' ? L.descriptionAr : L.descriptionEn) ??
          L.descriptionAr ??
          L.descriptionEn) ? (
            <section aria-labelledby="desc">
              <h2 id="desc" className="mb-2 text-h2">
                {t('description')}
              </h2>
          ...
          <section
            aria-labelledby="dealer"
            className="flex flex-col gap-3 rounded-lg border border-border bg-card p-4"
          >
            <h2 id="dealer" className="text-h2">
              {t('dealer')}
            </h2>
            <Link
              href={`/dealers/${d.dealer.slug}`}
            ...
            {d.dealer.responseRate !== null ? (
              <p className="text-caption">
                {t('responseRate', { percent: Math.round(d.dealer.responseRate * 100) })}
              </p>
            ) : null}
            {d.dealer.reviews.count ? (
              <p className="text-caption">{`★ ${d.dealer.reviews.average} · ${tu('reviewsCount', { count: d.dealer.reviews.count })}`}</p>
            ) : null}
            <p className="text-caption text-fg-secondary">
              {t('memberSince', {
                date: formatDate(new Date(d.dealer.memberSince), l, {
                  month: 'long',
                  year: 'numeric',
                }),
              })}
      ...
      {d.similar.length ? (
        <section aria-labelledby="similar">
          <h2 id="similar" className="mb-4 text-h2">
            {t('similar')}
          </h2>
