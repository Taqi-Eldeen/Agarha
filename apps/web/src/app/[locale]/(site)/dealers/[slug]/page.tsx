...
export async function generateMetadata({ params }: P): Promise<Metadata> {
  ...
  const city = p.branches[0]?.area
    ? l === 'ar'
      ? p.branches[0].area.cityNameAr
      : p.branches[0].area.cityNameEn
    : '';
  return {
    title: t('dealerTitle', { dealer: l === 'ar' ? p.dealer.nameAr : p.dealer.nameEn, city }),
    alternates: alternates(locale, `/dealers/${slug}`),
  };
}
  ...
  return (
    <div className="flex flex-col gap-8">
      <JsonLdScript
        data={dealerJsonLd({
          name,
          url: `${siteUrl}/${locale}/dealers/${slug}`,
          phone: p.dealer.phone,
          branches: p.branches.map((b) => ({
            name: l === 'ar' ? b.nameAr : b.nameEn,
            lat: b.lat,
            lng: b.lng,
            address: l === 'ar' ? b.addressAr : b.addressEn,
          })),
          rating: p.reviews,
        })}
      />
      <header className="flex flex-col gap-2">
        ...
        {p.responseRate !== null ? (
          <p>{tl('responseRate', { percent: Math.round(p.responseRate * 100) })}</p>
        ) : null}
        <p className="text-caption text-fg-secondary">
          {tl('memberSince', {
            date: formatDate(new Date(p.dealer.memberSince), l, { month: 'long', year: 'numeric' }),
          })}
        </p>
        {(l === 'ar' ? p.dealer.descriptionAr : p.dealer.descriptionEn) ? (
      ...
      <section aria-labelledby="branches">
        <h2 id="branches" className="mb-3 text-h2">
          {t('branches')}
        </h2>
          ...
          {p.branches.map((b) => (
            ...
            >
              ...
              <p className="flex items-center gap-1 text-caption text-fg-secondary">
                ...
                {[
                  l === 'ar' ? b.addressAr : b.addressEn,
                  b.area ? (l === 'ar' ? b.area.nameAr : b.area.nameEn) : null,
                ]
                  .filter(Boolean)
                  .join('، ')}
              </p>
      ...
      <section aria-labelledby="fleet">
        <h2 id="fleet" className="mb-3 text-h2">
          {t('fleet', { count: p.fleetTotal })}
        </h2>
      ...
      <section aria-labelledby="reviews">
        <h2 id="reviews" className="mb-3 text-h2">
          {t('reviews')}
        </h2>
        {p.reviews.latest.length ? (
          p.reviews.latest.map((r) => (
            <ReviewItem
              key={r.id}
              rating={r.rating}
              body={r.body}
          ...
          <p className="text-fg-secondary">{t('noReviews')}</p>
        )}
      </section>
    </div>
  );
