...
export async function generateMetadata({
  ...
  return {
    ...
    alternates: alternates(locale, '/for-dealers'),
  };
}
    ...
    <div className="flex flex-col gap-10">
      <section className="flex flex-col items-start gap-4 rounded-lg bg-brand-subtle p-6 md:p-10">
        ...
        <div className="flex flex-wrap gap-3">
          <Link href="/dealer/sign-up" className={buttonVariants({ size: 'lg' })}>
            {t('cta')}
          </Link>
          <Link
            href="/dealer/sign-in"
      ...
      <section aria-labelledby="benefits">
        <h2 id="benefits" className="mb-4 text-h2">
          {t('benefitsTitle')}
        </h2>
      ...
      <section aria-labelledby="steps">
        <h2 id="steps" className="mb-4 text-h2">
          {t('stepsTitle')}
        </h2>
        <ol className="grid gap-3 md:grid-cols-4">
          {(['step1', 'step2', 'step3', 'step4'] as const).map((s, i) => (
            ...
            >
              <span className="ag-tabular flex size-9 shrink-0 items-center justify-center rounded-full bg-brand font-semibold text-white dark:text-page">
                {i + 1}
              </span>
              {t(s)}
            </li>
