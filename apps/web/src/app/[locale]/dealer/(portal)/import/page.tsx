...
export default function Import() {
  ...
  return (
    <div className="flex max-w-2xl flex-col gap-4">
      ...
      <a
        className="self-start text-brand underline"
        href={`data:text/csv;charset=utf-8,${encodeURIComponent(TEMPLATE)}`}
        download="agarha-cars-template.csv"
      >
        {t('template')}
      </a>
      <label className="inline-flex min-h-touch cursor-pointer items-center self-start rounded-md border border-border px-4 hover:bg-brand-subtle">
        ...
        <input
          ...
          onChange={async (e) => {
            ...
            setBusy(true);
            try {
              const res = await fetch(`${env.NEXT_PUBLIC_API_URL}/v1/dealer/listings/import`, {
                method: 'POST',
                credentials: 'include',
        ...
        <InlineAlert tone="danger" title={t('errors', { count: v.errors.length })}>
          <ul>
            {v.errors.slice(0, 50).map((er) => (
              <li key={`${er.row}-${er.field}`}>
                {t('row', { row: er.row, field: er.field, code: er.code })}
              </li>
            ))}
          </ul>
        </InlineAlert>
      ...
      {v && v.status === 'ready' ? (
        <>
          ...
          <Button
            ...
            onClick={async () => {
              try {
                const { data } = await api.POST('/v1/dealer/listings/import/{id}/apply', {
                  params: { path: { id: v.importId } },
                });
                toast({
                  tone: 'success',
                  text: t('applied', { count: (data as unknown as { created: number }).created }),
                });
                setV(null);
              } catch (err) {
