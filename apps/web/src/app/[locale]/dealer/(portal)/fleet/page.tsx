...
export default function Fleet() {
  ...
  const fleet = useQuery({
    queryKey: ['fleet'],
    queryFn: async () =>
      (await api.GET('/v1/dealer/listings', { params: { query: { limit: 100 } } }))
        .data as unknown as { items: FleetItem[]; counts: Record<string, number> },
  });
  const items = fleet.data?.items ?? [];
  const needsConfirm = items.filter(
    (i) => (i.status === 'live' && i.freshness !== 'fresh') || i.hiddenReason === 'stale',
  ).length;
  const verified = me.data?.dealer.status === 'verified';

  const act = async (id: string, action: 'publish' | 'pause' | 'archive') => {
    try {
      await api.POST(`/v1/dealer/listings/{id}/${action}` as '/v1/dealer/listings/{id}/publish', {
        params: { path: { id } },
      });
  ...
  return (
    <div className="flex flex-col gap-4">
        ...
        <div className="flex gap-2">
          <Button
            ...
            onClick={async () => {
              ...
              toast({
                tone: 'success',
                text: t('confirmAllDone', {
                  count: (data as unknown as { confirmed: number }).confirmed,
                }),
              });
              await qc.invalidateQueries({ queryKey: ['fleet'] });
            }}
      ...
      {!verified && me.data ? (
        <InlineAlert tone="info">{t('publishNeedsVerify')}</InlineAlert>
      ) : null}
      {needsConfirm ? (
        <InlineAlert tone="warning">{t('confirmBanner', { count: needsConfirm })}</InlineAlert>
      ) : null}
      {fleet.isError ? (
        <ErrorState body={tu('errorTitle')} onRetry={() => void fleet.refetch()} />
      ) : null}
      {fleet.isSuccess && !items.length ? (
        <EmptyState
          body={t('empty')}
          action={
            <Link href="/dealer/fleet/new" className={buttonVariants({})}>
      ...
      <ul className="flex flex-col gap-3">
        {items.map((l) => {
          const name = l.model
            ? `${locale === 'ar' ? l.model.makeNameAr : l.model.makeNameEn} ${locale === 'ar' ? l.model.nameAr : l.model.nameEn}`
            : '';
          ...
          return (
            ...
            >
              <div className="flex flex-1 gap-3">
                <div className="size-20 shrink-0 overflow-hidden rounded-md bg-brand-subtle">
                ...
                <div className="flex min-w-0 flex-col gap-1">
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <span className="text-caption text-fg-secondary">
                      {t('lastConfirmed', {
                        ago: formatRelative(new Date(l.lastConfirmedAt), locale),
                      })}
                    </span>
                  </div>
                  {l.hiddenReason === 'stale' ? (
                    <p className="text-caption text-stale">{t('hiddenStale')}</p>
                  ) : null}
                </div>
              </div>
              <div className="flex flex-wrap items-center gap-2">
                {l.status === 'live' ? (
                  <AvailabilitySwitch
                    ...
                    onChange={async (available) => {
                      await api.PUT('/v1/dealer/listings/{id}/availability', {
                        params: { path: { id: l.id } },
                        body: { available },
                      });
                    ...
                    onChanged={(v, undo) =>
                      toast({
                        tone: 'success',
                        text: `${tu('availabilityUpdated')}: ${v ? tu('available') : tu('unavailable')}`,
                        action: { label: tu('undo'), onClick: undo },
                      })
                    }
                  />
                ) : null}
