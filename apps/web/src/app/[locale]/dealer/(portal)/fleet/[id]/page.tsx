...
export default function EditCar() {
  ...
  const q = useQuery({
    queryKey: ['dealer-listing', id],
    queryFn: async () =>
      (await api.GET('/v1/dealer/listings/{id}', { params: { path: { id } } }))
        .data as unknown as Listing & { model: ModelRef | null },
  });
  if (q.isLoading) return <ListingCardSkeleton />;
  if (q.isError || !q.data)
    return <ErrorState body={tu('errorTitle')} onRetry={() => void q.refetch()} />;
