...
export default function Stats() {
  ...
  const q = useQuery({
    queryKey: ['stats', days],
    queryFn: async () =>
      (await api.GET('/v1/dealer/stats', { params: { query: { days: Number(days) } } }))
        .data as DealerStats,
  });
  const s = q.data;
  const n = (v: number | null | undefined, suffix = '') =>
    v === null || v === undefined ? '—' : `${formatNumber(v, locale)}${suffix}`;
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <ChipGroup
        ...
        single
        value={[days]}
        onChange={(v) => v[0] && setDays(v[0])}
        options={['7', '30', '90'].map((d) => ({ value: d, label: t('period', { days: d }) }))}
      />
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
        ...
        <StatTile
          label={t('contacts')}
          value={n((s?.totals.whatsapp ?? 0) + (s?.totals.calls ?? 0))}
        />
      ...
      {s?.daily.length ? (
        <figure
          aria-label={t('views')}
          className="flex h-32 items-end gap-1 rounded-lg border border-border bg-card p-3"
          dir="ltr"
        >
          {s.daily.map((d) => (
            <div
        ...
        columns={[
          {
            key: 'car',
            header: t('perCar'),
            cell: (r) => `${locale === 'ar' ? r.nameAr : r.nameEn} ${r.year}`,
          },
          {
            key: 'views',
            header: t('views'),
          ...
          },
          {
            key: 'contacts',
            header: t('contacts'),
            cell: (r) => n(r.whatsapp + r.calls),
            sortValue: (r) => r.whatsapp + r.calls,
            numeric: true,
          },
          {
            key: 'conv',
            header: t('conversion'),
            cell: (r) => n(r.conversion, '%'),
          ...
          },
          {
            key: 'fresh',
            header: t('freshness'),
            cell: (r) => n(r.freshnessScore, '%'),
            sortValue: (r) => r.freshnessScore,
            numeric: true,
          },
        ]}
      />
