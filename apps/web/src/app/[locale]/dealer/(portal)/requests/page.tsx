...
type Req = {
  ...
  endDate: string;
  note: string | null;
  status: 'sent' | 'available' | 'unavailable' | 'expired';
};
...
export default function Requests() {
  ...
  const q = useQuery({
    queryKey: ['requests'],
    queryFn: async () =>
      (await api.GET('/v1/dealer/availability-requests')).data as unknown as { items: Req[] },
  });
  const answer = async (id: string, available: boolean) => {
    await api.POST('/v1/dealer/availability-requests/{id}/answer', {
      params: { path: { id } },
      body: { available },
    });
    await q.refetch();
    ...
    <div className="flex flex-col gap-4">
      ...
      <ul className="flex flex-col gap-2">
        {q.data?.items.map((r) => (
          ...
          >
            <div>
              <p dir="ltr" className="font-mono text-caption">
                {r.refCode}
              </p>
            ...
            {r.status === 'sent' ? (
              <div className="flex gap-2">
                ...
                <Button size="sm" variant="secondary" onClick={() => void answer(r.id, false)}>
                  {t('no')}
                </Button>
              </div>
            ) : null}
