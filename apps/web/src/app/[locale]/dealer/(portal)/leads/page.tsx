...
type Lead = {
  ...
  createdAt: string;
  signedIn: boolean;
  listing: { nameAr: string; nameEn: string; year: number } | null;
};
...
export default function Leads() {
  ...
  const q = useQuery({
    queryKey: ['leads'],
    queryFn: async () =>
      (await api.GET('/v1/dealer/leads', { params: { query: { limit: 100 } } }))
        .data as unknown as { items: Lead[] },
  });
      ...
      <DataTable
        ...
        columns={[
          ...
          },
          {
            ...
            cell: (r) =>
              r.listing
                ? `${locale === 'ar' ? r.listing.nameAr : r.listing.nameEn} ${r.listing.year}`
                : '—',
          },
          {
            key: 'channel',
            header: t('channel'),
            cell: (r) =>
              `${r.channel === 'whatsapp' ? t('whatsapp') : t('call')}${r.signedIn ? ` · ${t('signedIn')}` : ''}`,
          },
          {
            key: 'time',
            header: t('time'),
            ...
            cell: (r) => (
              <Select
                ...
                onValueChange={async (v) => {
                  if (v === 'unknown') return;
                  await api.PUT('/v1/dealer/leads/{id}/outcome', {
                    params: { path: { id: r.id } },
                    body: { outcome: v as 'rented' },
                  });
