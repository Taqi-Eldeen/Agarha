...
const PATHS = ['/ar/cars/*', '/en/cars/*', '/ar/dealers/*', '/en/dealers/*', '/ar/search*', '/en/search*'];

export function GET() {
  const team = process.env.APPLE_TEAM_ID;
  const bundle = process.env.IOS_BUNDLE_ID ?? 'com.agarha.app';
  const body = team ? { applinks: { details: [{ appIDs: [`${team}.${bundle}`], components: PATHS.map((p) => ({ '/': p })) }] } } : { applinks: { details: [] } };
