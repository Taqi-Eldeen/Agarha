...
interface SafeParser {
  safeParse(input: unknown): { success: true; data: unknown } | { success: false; error: { issues: { path: PropertyKey[]; message: string; code: string; minimum?: unknown; input?: unknown }[] } };
}

/** Maps a Zod issue to an `errors.*` catalog key: a custom message code wins, else the issue kind. */
export function issueKey(i: { message: string; code: string; minimum?: unknown; input?: unknown }, known: (k: string) => boolean): string {
  if (known(i.message)) return i.message;
  if (i.input === undefined || i.input === '' || (i.code === 'too_small' && Number(i.minimum) <= 1)) return 'required';
 ...
 */
export function schemaResolver<T extends FieldValues>(schema: SafeParser, toInput: (v: T) => unknown = (v) => v, known: (k: string) => boolean = () => false): Resolver<T> {
  return async (values) => {
...
export function useFieldError() {
  ...
  const text = (message?: string) => (message ? (known(message) ? te(message as never) : te('validation_failed')) : undefined);
  return { text, known };
}
...
export function applyServerErrors<T extends FieldValues>(form: UseFormReturn<T>, err: unknown): boolean {
  const fe = err instanceof ApiRequestError ? (err.details as { fieldErrors?: Record<string, string[]> } | undefined)?.fieldErrors : undefined;
  if (!fe) return false;
  for (const [k, v] of Object.entries(fe)) form.setError(k as Path<T>, { type: 'server', message: v[0] ?? 'validation_failed' });
  return true;
}
