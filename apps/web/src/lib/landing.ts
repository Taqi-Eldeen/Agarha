...
export function landingTitle(locale: 'ar' | 'en', d: Landing): string {
  ...
  if (d.area)
    return interpolate(m.web.landing.areaTitle, {
      area: locale === 'ar' ? d.area.nameAr : d.area.nameEn,
      city,
    });
  if (d.type)
    return interpolate(m.web.landing.typeTitle, {
      type: m.ui.bodyTypes[d.type as keyof typeof m.ui.bodyTypes],
      city,
    });
...
export function landingIntro(locale: 'ar' | 'en', d: Landing): string {
  ...
  return d.stats.n
    ? interpolate(m.intro, { count: d.stats.n, min: formatEgp(d.stats.min ?? 0, locale) })
    : m.introEmpty;
}
...
export function landingMeta(locale: string, d: Landing, path: string): Metadata {
  ...
  return {
    title: `${landingTitle(l, d)} | ${l === 'ar' ? 'أجّرها' : 'Agarha'}`,
    description: landingIntro(l, d),
    alternates: alternates(locale, path),
    robots: d.stats.n ? undefined : { index: false },
  };
}
