...
export function listingJsonLd(d: ListingDetail, locale: 'ar' | 'en', url: string) {
  ...
  return {
    ...
    offers: {
      ...
      availability: d.card.available
        ? 'https://schema.org/InStock'
        : 'https://schema.org/OutOfStock',
      seller: {
        '@type': 'AutoRental',
        name: locale === 'ar' ? d.dealer.nameAr : d.dealer.nameEn,
        url: `${siteUrl}/${locale}/dealers/${d.dealer.slug}`,
      },
      areaServed: d.card.city.en,
    },
    ...(d.dealer.reviews.average !== null && d.dealer.reviews.count > 0
      ? {
          aggregateRating: {
            '@type': 'AggregateRating',
            ratingValue: d.dealer.reviews.average,
            reviewCount: d.dealer.reviews.count,
          },
        }
      : {}),
  };
...
export function dealerJsonLd(p: {
  name: string;
  url: string;
  phone: string;
  branches: { name: string; lat: number | null; lng: number | null; address: string | null }[];
