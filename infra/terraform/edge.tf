  ...
  rules = [
    {
      ...
      expression  = "(http.host eq \"${local.hosts.admin}\" or (http.host eq \"${local.hosts.api}\" and starts_with(http.request.uri.path, \"/v1/admin\"))) and not ip.src in {${join(" ", var.admin_ip_allowlist)}}"
      action      = "block"
    },
    {
      description = "No bulk scraping of the search API by non-browser clients"
      expression  = "http.host eq \"${local.hosts.api}\" and starts_with(http.request.uri.path, \"/v1/search\") and cf.bot_management.verified_bot eq false and cf.client.bot"
  ...
  rules = [
    {
      ...
      expression  = "http.host eq \"${local.hosts.api}\" and starts_with(http.request.uri.path, \"/v1/auth/otp\")"
      action      = "block"
    ...
    },
    {
      ...
      expression  = "http.host eq \"${local.hosts.api}\" and http.request.uri.path eq \"/v1/leads\""
      action      = "block"
    ...
    },
    {
      ...
      expression  = "http.host eq \"${local.hosts.api}\" and (starts_with(http.request.uri.path, \"/v1/search\") or starts_with(http.request.uri.path, \"/v1/listings\"))"
      action      = "managed_challenge"
